...
#  endif                          /* TOS */
      ...
      /* See if the last file got there.  If so, remove the ramdisk
       * bones files. */
...
#ifdef TOS
   if (run_from_desktop)
      getreturn ("to continue");/* so the user can read the score
                                 * list */
# ifdef TEXTCOLOR
   if (colors_changed)
      restore_colors ();
...
#endif
#ifdef WIN32CON
   /* Only if we started from Progman, not command prompt, we need to
    * get one last return, so the score board does not vanish
    * instantly after being created. ProgmanLaunched is defined and
    * set in nttty.c. */
