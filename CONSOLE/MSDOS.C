 ...
 * MS-DOS functions
 */
#define DIRECT_INPUT    0x07     /* Unfiltered Character Input Without
                                 * Echo */
...
#ifdef PC9801
   if (ch < 0)                    /* KANA letters and GRPH-shifted
                                 * letters(?) */
      ch = 0;                    /* munch it */
#else
   /* The extended codes for Alt-shifted letters, and unshifted
    * keypad and function keys, correspond to the scan codes.  So we
    * can still translate the unshifted cursor keys and Alt-letters.
    * -3. */
   if (ch == 0)
      {                             /* an extended key */
      regs.h.ah = DIRECT_INPUT;
