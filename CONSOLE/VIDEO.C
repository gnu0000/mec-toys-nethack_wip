 ...
 * void tty_startup (wid, hgt)
 * void tty_start_screen ()
 * void xputs (s)
 * void xputc (ch)                 /* write out character (and
 * 
 * void  get_cursor (x, y)           /* get cursor position */
 * void  gotoxy (x, y)
 * int   monoadapt_check ()
...
//    * 
//    * Otherwise, DL now contains rows - 1.  Also, CX contains the points
//    * (bytes per character) and ES:BP points to the font table.  -3. */
//
//   regs.h.ah = GETMODE;
//   int86 (VIDEO_BIOS, &regs, &regs);   /* Get Video Mode */
//
//   /* This goes back all the way to the original PC.  Completely
//    * safe.  AH contains # of columns, AL contains display mode, and
//    * BH contains the active display page. */
//
//   LI = regs.h.dl + 1;
//   CO = regs.h.ah;
//# endif                          /* PC9801 */
...
# ifdef MONO_CHECK
int   monoflag;                 /* 0 = not monochrome, else
                                 * monochrome */
...
# endif
   ...
   /* This, too, goes back all the way to the original PC.  If we
    * ever get so fancy as to swap display pages (i doubt it), then
    * we'll need to set BH appropriately.  This function returns
    * nothing.  -3. */
   }
...
int   assign_videocolors (uchar * tmpcolor, int len)
   ...
   init_ttycolor ();              /* in case nethack.cnf entry wasn't
                                 * complete */
   icolor = RED;
   for (i = 0; i < len; ++i)
      {
