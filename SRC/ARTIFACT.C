...
struct obj *otmp;                 /* existing object; ignored if
                                 * alignment specified */
   ...
   if (n)
      ...
      for (a = artilist + 1, m = 1; a->otyp; a++, m++)
         if ((by_align ? a->alignment == alignment : a->otyp == o_typ) &&
            ...
            {
            if (!--n)
               break;           /* stop when chosen one reached */
            }
...
make_artif:if (by_align)
      ...
      {
      /* nothing appropriate could be found; return the original
       * object */
      if (by_align)
...
boolean
restrict_name (otmp, name)        /* returns 1 if name is restricted
                                 * for otmp->otyp */
register struct obj *otmp;
register const char *name;
   {
   ...
   /* Since almost every artifact is SPFX_RESTR, it doesn't cost us
    * much to do the string comparison before the spfx check. Bug
    * fix:  don't name multiple elven daggers "Sting". */
   for (a = artilist + 1; a->otyp; a++)
      if (a->otyp == otmp->otyp && !strcmp (a->name, name))
...
long  wp_mask;
   ...
   if (spfx & SPFX_HALRES)
      {
      /* make_hallucinated must (re)set the mask itself to get the
       * display right */
...
struct monst *mon;
   ...
   /* can pick it up unless you're totally non-synch'd with the
    * artifact */
   if (badclass && badalign && (oart->spfx & SPFX_INTEL))
      {
      if (yours)
         pline ("%s evades your grasp!", The (xname (obj)));
      return 0;
      }
...
struct obj *otmp;
int  *dmgptr;
int   dieroll;                    /* needed for Magicbane and vorpal
                                 * blades */
   {
   ...
   *dmgptr += spec_dbon (otmp, youdefend ? &playermon
    ...
    * dependence on spe.  Against low mr victims, it typically does
    * "double athame" damage, 2d4.  Occasionally, it will cast
    * unbalancing magic which effectively averages out to 4d4 damage
    * (2.5d4 against high mr victims), for spe = 0. */
...
#endif
            /* Players with negative AC's take less damage instead of
             * just not getting hit.  We must add a large enough
             * value to the damage so that this reduction in damage
   ...
   if (oart->inv_prop > LAST_PROP)
      ...
      switch (oart->inv_prop)
         ...
         case UNTRAP:
               ...
               if (!untrap (TRUE))
                  {
                  obj->age = 0;  /* don't charge for changing their
                                 * mind */
                  return 0;
                  }
               break;
         ...
         case CREATE_PORTAL:
               ...
               /* run thru dungeon array to find the one they
                * selected */
               for (i = 0; hc >= 'a'; i++)
                  if (dungeons[i].dunlev_ureached)
                     hc--;
               ...
               /* i is now index into dungeon structure for the new
                * dungeon. Find the closest level in the given
                * dungeon, open a use-once portal to that dungeon and
                * go there. The closest level is either the entry or
                * dunlev_ureached. */
               newlev.dnum = i;
      ...
      boolean on = (cprop & W_ARTI) != 0;   /* true if invoked prop
                                           * just set */
