...
register struct monst *mtmp;
   ...
   mtmp->mpeaceful = 1;
   set_malign (mtmp);           /* recalc alignment now that it's
                                 * tamed */
   mtmp->mleashed = 0;
   mtmp->meating = 0;
   ...
   switch (pl_character[0])
      ...
      default:
         if (preferred_pet == 'c')
            ...
            pettype = rn2 (2) ? PM_KITTEN : PM_LITTLE_DOG;
         break;
      }
   return pettype;
   }
...
top:
   ...
   if (!mtmp)
       ...
       * either rndmonst() is called, or if only one of cat/dog was
       * genocided, they get the other. */
      return;
      }
   initedog (mtmp);
   mtmp->msleep = 0;
...
void
   ...
   for (mtmp = migrating_mons; mtmp; mtmp = mtmp2)
      ...
      if (mtmp->mx == u.uz.dnum && mtmp->mux == u.uz.dlevel)
         ...
         mtmp->mux = u.ux;
         mtmp->muy = u.uy;        /* not really req'd */
         ...
         if (mtmp->mlstmv < monstermoves - 1)
            ...
            if (nmv >= (long) mtmp->mblinded)
               mtmp->mblinded = 1;
            else
               mtmp->mblinded -= nmv;
            if (nmv >= (long) mtmp->mfrozen)
               mtmp->mfrozen = 1;
            else
               mtmp->mfrozen -= nmv;
            if (nmv >= (long) mtmp->mfleetim)
               mtmp->mfleetim = 1;
            else
               mtmp->mfleetim -= nmv;

            /* might be able to use special ability again */
            if (nmv > (long) mtmp->mspec_used)
               mtmp->mspec_used = 0;
            else
               mtmp->mspec_used -= nmv;
            ...
            mtmp->mx = min (nmv, 8);
#ifdef MULDGN
            /* temporary hack... Ft.Ludios' entry chamber isn't
             * classified as a room by lev_comp and any movement
             * greater than 1 can result in placement beyond solid
             * walls. */
            if (Is_knox (&u.uz))
               mtmp->mx = 1;
#endif
...
void
   ...
   for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
       ...
       * if you don't have it, will chase you only if in range. -3. */
      ...
      merge:
         ...
         relmon (mtmp);
         newsym (mtmp->mx, mtmp->my);
         mtmp->mx = mtmp->my = 0;   /* avoid mnexto()/MON_AT()
                                     * problem */
...
xchar xyloc;                    /* destination xy location: */
 ...
 /* 0: rnd, 1: <, 2: >, 3: < ladder, 4: > ladder, 5: sstairs 6:
  * portal */
   {
   register struct obj *obj;
