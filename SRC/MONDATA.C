...
boolean
resists_drli (ptr)              /* returns TRUE if monster is
...
boolean
ranged_attk (ptr)                 /* returns TRUE if monster can attack
                                 * at range */
register struct permonst *ptr;
   {
   register int i,
    ...
    * attacktype(ptr, AT_MAGC)); but that's too slow -dlc */
   for (i = 0; i < NATTK; i++)
      {
...
boolean
can_track (ptr)                 /* returns TRUE if monster can track
                                 * well */
register struct permonst *ptr;
   {
...
boolean
sticks (ptr)                    /* creature sticks other creatures it
                                 * hits */
register struct permonst *ptr;
   {
...
int
      monsndx (ptr)              /* return an index into the mons
                                 * array */
struct permonst *ptr;
   {
   register int i;
...
char *str;
    ...
    * already been taken off the front, so we have to be able to read
    * the name with extraneous stuff such as "corpse" stuck on the
    * end. This causes a problem for names which prefix other names
    * such as "ettin" on "ettin zombie".  In this case we want the
    * _longest_ name which exists. This also permits plurals created
    * by adding suffixes such as 's' or 'es'.  Other plurals must
   ...
   if (!strncmpi (str, "baby grey dragon", 16))
      return PM_BABY_GRAY_DRAGON;
   if (!strncmpi (str, "grey unicorn", 12))
      return PM_GRAY_UNICORN;
   if (!strncmpi (str, "grey ooze", 9))
      return PM_GRAY_OOZE;

   /* Some irregular plurals */
   if (!strncmpi (str, "incubi", 6))
      return PM_INCUBUS;
   if (!strncmpi (str, "succubi", 7))
      return PM_SUCCUBUS;
   if (!strncmpi (str, "violet fungi", 12))
      return PM_VIOLET_FUNGUS;
   if (!strncmpi (str, "homunculi", 9))
      return PM_HOMUNCULUS;
   if (!strncmpi (str, "baluchitheria", 13))
      return PM_BALUCHITHERIUM;
   if (!strncmpi (str, "lurkers above", 13))
      return PM_LURKER_ABOVE;
   if (!strncmpi (str, "cavemen", 7))
      return PM_CAVEMAN;
   if (!strncmpi (str, "cavewomen", 9))
      return PM_CAVEWOMAN;
   if (!strncmpi (str, "zruties", 7))
      return PM_ZRUTY;
   if (!strncmpi (str, "djinn", 5))
      return PM_DJINNI;
   if (!strncmpi (str, "mumakil", 7))
      return PM_MUMAK;
...
struct permonst *
   ...
   switch (pl_character[0])
      ...
      case 'B':
         return &mons[PM_BARBARIAN];
      case 'C':
         if (flags.female)
            return &mons[PM_CAVEWOMAN];
         else
            return &mons[PM_CAVEMAN];
      case 'E':
         return &mons[PM_ELF];
      case 'H':
         return &mons[PM_HEALER];
      case 'K':
         return &mons[PM_KNIGHT];
      case 'P':
         if (flags.female)
            return &mons[PM_PRIESTESS];
         else
            return &mons[PM_PRIEST];
      case 'R':
         return &mons[PM_ROGUE];
      case 'S':
         return &mons[PM_SAMURAI];
#ifdef TOURIST
      case 'T':
         return &mons[PM_TOURIST];
#endif
      case 'V':
         return &mons[PM_VALKYRIE];
      case 'W':
         return &mons[PM_WIZARD];
      default:
