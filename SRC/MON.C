...
struct monst *fdmon;              /* chain of dead monsters, need not
                                 * be saved */
   ...
   default_1:
      default:
         if (mdat->geno & G_NOCORPSE)
            ...
            obj = mkcorpstat (CORPSE, mdat, x, y, TRUE);
         break;
      }
   /* All special cases should precede the G_NOCORPSE check */

   /* Note: oname() cannot be used generically for non-inventory
    * objects unless you fix the link from the previous object in the
    * chains. (Here we know it's the first one, so there was no
    * link.) */
   if (mtmp->mnamelth)
      {
...
register struct monst *mtmp;
   ...
   if (inpool)
      {
      /* Most monsters drown in pools.  flooreffects() will take care
       * of water damage to dead monsters' inventory, but survivors
       * need to be handled here.  Swimmers are able to protect their
       * stuff... */
...
void
   ...
   while (1)
      {
      /* Find a monster that we have not treated yet. Note that mtmp
       * or mtmp->nmon might get killed while mtmp moves, so we
       * cannot just walk down the chain (even new monsters might get
       * created!)
       * 
...
next_mon:
      ...
      if (mtmp->mspeed != MSLOW || !(moves % 2))
         ...
         if (Conflict && !mtmp->iswiz && mtmp->mcansee)
            {
            /* Note: Conflict does not take effect in the first
             * round. Therefore, A monster when stepping into the
             * area will get to swing at you.
             * 
...
register struct monst *mtmp;
   ...
   for (otmp = level.objects[mtmp->mx][mtmp->my];
      ...
      if (is_metallic (otmp) && !obj_resists (otmp, 5, 95) &&
         ...
         /* Left behind a pile? */
         if (rnd (25) < 3)
            (void) mksobj_at (ROCK, mtmp->mx, mtmp->my, TRUE);
...
register struct monst *mtmp;
    ...
    * carrying capacity, or half human maximum if not strong. (for a
    * polymorphed player, the value used would be the non-polymorphed
    * carrying capacity instead of max/half max). This is then
    * modified by the ratio between the monster weights and human
    * weights.  Corpseless monsters are given a capacity proportional
    * to their size instead of weight. */
   if (!mtmp->data->cwt)
...
nexttry:                       /* eels prefer the water, but if
                                 * there is no water nearby, they
   ...
   for (nx = max (1, x - 1); nx <= maxx; nx++)
      for (ny = max (0, y - 1); ny <= maxy; ny++)
         ...
         if (IS_ROCK (ntyp = levl[nx][ny].typ) &&
             ...
             !((flag & ALLOW_DIG) && may_dig (nx, ny)))
            continue;
         if (IS_DOOR (ntyp) && !amorphous (mon->data) &&
         ((levl[nx][ny].doormask & D_CLOSED && !(flag & OPENDOOR)) ||
          (levl[nx][ny].doormask & D_LOCKED && !(flag & UNLOCKDOOR))
          ) && !(flag & (ALLOW_WALL | ALLOW_DIG | BUSTDOOR)))
            continue;
         if (nx != x && ny != y && (nodiag ||
#ifdef REINCARNATION
                                    ((IS_DOOR (nowtyp) &&
         ...
         if ((is_pool (nx, ny) == wantpool || poolok) &&
            ...
            /* Displacement also displaces the Elbereth/scare
             * monster, as long as you are visible. */
            ...
            if ((nx == u.ux && ny == u.uy) ||
               ...
               if (nx == u.ux && ny == u.uy)
                   ...
                   * displaced or no.  We must set mux and muy right
                   * now, so when we return we can tell that the
                   * ALLOW_U means to attack _you_ and not the image. */
                  mon->mux = u.ux;
                  mon->muy = u.uy;
                  }
               if (!(flag & ALLOW_U))
...
register struct monst *mtmp,
   ...
   if (mtmp2->wormno)           /* update
                                 * level.monsters[wseg->wx][wseg->wy] */
...
register struct monst *mtmp;
   ...
   mtmp->mtrapped = 0;
   mtmp->mhp = 0;                 /* simplify some tests: force mhp to
                                 * 0 */
   relobj (mtmp, 0, FALSE);
   relmon (mtmp);
   newsym (mtmp->mx, mtmp->my);
...
register struct monst *mtmp;
   ...
   if (mtmp->isgd)
      {
      /* if we're going to abort the death, it *must* be before the
       * m_detach or there will be relmon problems later */
      if (!grddead (mtmp))
         return;
      }
#ifdef MUSE
   ...
   /* if MAXMONNO monsters of a given type have died, and it can be
    * done, extinguish that monster.
    * 
    * u.nr_killed does double duty as total number of dead monsters and
    * as experience factor for the player killing more monsters. this
    * means that a dragon dying by other means reduces the experience
    * the player gets for killing a dragon directly; this is probably
    * not too bad, since the player likely finagled the first dead
    * dragon via ring of conflict or pets, and extinguishing based on
    * only player kills probably opens more avenues of abuse for
...
#ifdef MUSE
   /* we have to make the statue before calling mondead, to be able
    * to put inventory in it, and we have to check for lifesaving
    * before making the statue.... */
...
int   dest;
   ...
   if (mtmp->mtrapped &&
       ...
       sobj_at (BOULDER, x, y))
      dest ^= 2;                 /* Prevent corpses/treasure being
                                 * created "on top" of the boulder
                                 * that is about to fall in. This is
                                 * out of order, but cannot be helped
                                 * unless this whole routine is
                                 * rearranged. */
...
#ifdef MUSE
   if (mtmp->mhp > 0)
      return;                    /* monster lifesaved */
#endif

   mdat = mtmp->data;           /* note: mondead can change
                                 * mtmp->data */
   ...
   if (!accessible (x, y))
       ...
       * different from whether or not the corpse is "special"; if we
       * want both, we have to specify it explicitly. */
...
xchar x,
      y;
boolean move_other;              /* make sure mtmp gets to x, y! so
                                 * move m_at(x, y) */
   {
   ...
   if (!goodpos (newx, newy, mtmp, mtmp->data))
       ...
       * migrating_mons that need to be placed will cause no end of
       * trouble. */
...
register struct monst *mtmp;
   {
   mtmp->msleep = 0;
   mtmp->meating = 0;           /* assume there's no salvagable food
                                 * left */
   setmangry (mtmp);
   if (mtmp->m_ap_type)
      seemimic (mtmp);
...
register struct permonst *mdat;
   ...
   if (In_endgame (&u.uz) && is_mplayer (olddata))
      {
      /* mplayers start out as "Foo the Bar", but some of the titles
       * are inappropriate when polymorphed, particularly into the
       * opposite sex.  players don't use ranks when polymorphed, so
       * dropping the rank for mplayers seems reasonable. */
...
/* and the same for maximum hit points */
   ...
   /* Unfortunately, by now we forgot who did the polymorph, so we
    * don't have any way to give the player credit if this was a
    * polymorph wand. */
