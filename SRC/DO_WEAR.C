...
int
   ...
   switch (uarmf->otyp)
      ...
      case LEVITATION_BOOTS:
         ...
         break;
      default:
         impossible ("Unknown type of boots (%d)", uarmf->otyp);
      }
   return 0;
   }
...
int
   ...
   /* For levitation, float_down() returns if Levitation, so we must
    * do a setworn() _before_ the levitation case. */
   long  oldprop = u.uprops[objects[uarmf->otyp].oc_oprop].p_flgs & ~WORN_BOOTS;
   ...
   switch (obj->otyp)
      ...
      case JUMPING_BOOTS:
         break;
      default:
         impossible ("Unknown type of boots (%d)", obj->otyp);
      }
   return 0;
   }
...
static int
   ...
   switch (uarmc->otyp)
      ...
      case OILSKIN_CLOAK:
         ...
         break;
      default:
         impossible ("Unknown type of cloak (%d)", uarmc->otyp);
      }
   return 0;
   }
...
int
   ...
   switch (uarmh->otyp)
      ...
      case HELM_OF_OPPOSITE_ALIGNMENT:
         ...
         break;
      default:
         impossible ("Unknown type of helm (%d)", uarmh->otyp);
      }
   return 0;
   }
   ...
   switch (uamul->otyp)
      ...
      case AMULET_OF_MAGICAL_BREATHING:
         if (Underwater)
            ...
            /* HMagical_breathing must be set off before calling
             * drown() */
            setworn ((struct obj *) 0, W_AMUL);
            (void) drown ();
            return;
...
register struct obj *otmp;
   ...
   if (!already_blinded)
      {
      if (Punished)
         set_bc (0);              /* Set ball&chain variables before */
      /* the hero goes blind.         */
      if (Telepat)
         see_monsters ();        /* sense monsters */
...
register struct obj *otmp;
   ...
   if (!Blinded)
      {
      if (Telepat)
         see_monsters ();        /* no longer sense monsters */
      vision_full_recalc = 1;     /* recalc vision limits */
      flags.botl = 1;
      }
   else
...
void
      cancel_don ()
   {
   /* the piece of armor we were donning/doffing has vanished, so
    * stop wasting time on it (and don't dereference it when donning
...
#define MOREACC(x) if (x)    { Accessories++; otmp = x;    }
   ...
   if (Accessories != 1)
      otmp = getobj (accessories, "take off");
   if (!otmp)
      return (0);
...
register struct obj *otmp;
   ...
   if (delay)
      ...
      {
      /* Be warned!  We want off_msg after removing the item to avoid
       * "You were wearing ____ (being worn)."  However, an item
       * which grants fire resistance might cause some trouble if
       * removed in Hell and lifesaving puts it back on; in this case
       * the message will be printed at the wrong time (after the
       * messages saying you died and were lifesaved).  Luckily, no
       * cloak, shield, or fast-removable armor grants fire
...
struct obj *
   ...
   if (uarmg && (!otmph || !rn2 (4)))
      otmph = uarmg;
   if (uarmf && (!otmph || !rn2 (4)))
      otmph = uarmf;
   if (uarms && (!otmph || !rn2 (4)))
      otmph = uarms;
#ifdef TOURIST
   if (!uarm && !uarmc && uarmu && (!otmph || !rn2 (4)))
      otmph = uarmu;
#endif
   return (otmph);
   }
...
register struct obj *otmp;
   ...
   if (cursed (otmp))
      return (0);
#ifdef POLYSELF
   if (otmp->oclass == RING_CLASS && nolimbs (uasmon))
      return (0);
#endif
   ...
   if (otmp == uarm)
      ...
      takeoff_mask |= 1L;        /* WIELDED_WEAPON */
...
static struct obj *
   ...
   if (taking_off == 1L)
      ...
      if (!cursed (ublindf))
         ...
         if (!Blinded)
            make_blinded (1L, FALSE);   /* See on next move */
         else
            You ("still cannot see.");
         }
      }
...
int
   ...
   if (taking_off == 0L)
      ...
      {
      otmp = uarm;
      /* If a cloak is being worn, add the time to take it off and
       * put it back on again.  Kludge alert! since that time is 0
       * for all known cloaks, add 1 so that it actually matters... */
      if (uarmc)
