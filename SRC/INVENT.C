...
static void
   ...
   do
      {
      /* We expect at most one item to be out of order, so this isn't
       * nearly as inefficient as it may first appear. */
...
register int lose;
   ...
   if (mergable (otmp, obj))
      {
      /* Approximate age: we do it this way because if we were to do
       * it "accurately" (merge only when ages are identical) we'd
       * wind up never merging any corpses. otmp->age =
       * otmp->age*(1-proportion) + obj->age*proportion; */
...
register struct obj *obj;
   ...
   if (obj->otyp == GOLD_PIECE)
      ...
      {
      if (u.uhave.questart)
         impossible ("already have the artifact?");
      u.uhave.questart = 1;
      artitouch ();
...
register struct obj *obj;
   ...
   if (obj->otyp == GOLD_PIECE)
      ...
      {
      if (!u.uhave.questart)
         impossible ("don't have the artifact?");
      u.uhave.questart = 0;
...
register struct obj *obj;
   ...
   if (obj->otyp == AMULET_OF_YENDOR ||
      ...
      {
      /* player might be doing something stupid, but we can't
       * guarantee that.  assume special artifacts are indestructible
       * via drawbridges, and exploding chests, and golem creation,
       * and ... */
      return;
      }
   freeobj (obj);
...
register const char *let,
   ...
   /* Two possibilities: they can't use gold because it's illegal, or
    * they can't use gold because they don't have any. */
   ...
   for (;;)
      ...
      if (ilet == def_oc_syms[GOLD_CLASS])
         ...
         if (cnt == 0 && prezero)
            return ((struct obj *) 0);
         /* Historic note: early Nethack had a bug which was first
          * reported for Larn, where trying to drop 2^32-n gold
          * pieces was allowed, and did interesting things to your
          * money supply.  The LRS is the tax bureau from Larn. */
         if (cnt < 0)
            {
   ...
   if (allowcnt == 2)
      ...
      if (cnt != otmp->quan)
          ...
          * to drop one of several loadstones and having the
          * loadstone now be separate. */
         if (!strcmp (word, "drop") &&
             obj->otyp == LOADSTONE && obj->cursed)
...
register int FDECL ((*fn), (OBJ_P)), mx;
   ...
   while ((sym = *ip++) != 0)
      ...
      else if (oc_of_sym != VENOM_CLASS)
         {                          /* venom doesn't show up */
   ...
   if (allowgold == 2 && !oletct)
      return 1;                 /* you dropped gold (or at least
                                 * tried to) */
   else
...
register int allflag,
      mx;
register const char *olets,
     *word;                       /* olets is an Obj Class char array */
register int FDECL ((*fn), (OBJ_P)), FDECL ((*ckfn), (OBJ_P));
    ...
    * For example, if a person specifies =/ then first all rings will
    * be asked about followed by all wands -dgk */
nextclass:
   ilet = 'a' - 1;
   if (*objchn && (*objchn)->otyp == GOLD_PIECE)
   ...
   for (otmp = *objchn; otmp; otmp = otmp2)
      ...
      if (takeoff && !is_worn (otmp))
         continue;
      if (ident && is_fully_identified (otmp))
         continue;
      if (ckfn && !(*ckfn) (otmp))
         continue;
      if (!allflag)
      ...
      if (sym == '#')
          ...
          * loadstones and welded weapons (eg, multiple daggers) will
          * remain as merged unit; done to avoid splitting an object
          * that won't be droppable (even if we're picking up rather
          * than dropping). */
         if (!yn_number)
            sym = 'n';
         else
...
STATIC_OVL char
      obj_to_let (obj)           /* should of course only be called
                                 * for things in invent */
register struct obj *obj;
   {
   register struct obj *otmp;
   register char ilet;
...
register long cost;              /* cost (for inventory of unpaid or
                                 * expended items) */
   {
   ...
   /* If let is: *  Then obj == NULL and we are printing a total
    * amount. >  Then the object is contained and doesn't have an
    * inventory letter. */
   if (cost != 0 || let == '*')
      {
...
boolean show_cost;
   ...
   if (lets != NULL)
      {
      int   ct = strlen (lets);  /* count number of inventory slots to
                                 * show */
      ...
      if (do_containers && ct == 1)
         ct = count_unpaid (invent);
      /* if only one item of interest, use pline instead of menus */
      if (ct == 1)
         {
         if (do_containers)
            {                       /* single non-inventory object */
            z_obj = (struct obj *) 0;
...
nextclass:
   ...
   if (do_containers)
       ...
       * unpaid items.  Note that we check each container, not the
       * invent list.  This is because the invent list could have
       * unpaid items that have been already listed. */
      for (otmp = invent; otmp; otmp = otmp->nobj)
         {
...
/* Changed to one type only, so you don't have to type return */
   ...
   /* collect a list of classes of objects carried, for use as a
    * prompt */
...
static boolean
      mergable (otmp, obj)        /* returns TRUE if obj  & otmp can be
                                 * merged */
register struct obj *otmp,
     *obj;
   {
...
boolean unpaid;
   ...
   if (len > bufsiz)
      {
      if (buf)
         free ((genericptr_t) buf), buf = NULL;
      bufsiz = len + 10;        /* add slop to avoid incremental
                                 * reallocations */
...
int
   ...
   allow_all[0] = ALL_CLASSES;
   allow_all[1] = '\0';
   if (!(obj = getobj (allow_all, "adjust")))
      return (0);
