...
static NEARDATA struct obj *book;   /* last/current book being
                                     * xscribed */
...
register int lev;
   ...
   switch (rn2 (lev))
      ...
      case 5:
         ...
         if (uarmg)
            {
            /* Note: at this writing, there are no corrodeable gloves
             * in the game.  If no one plans on adding copper
             * gauntlets, most of this could be removed. -3. */
...
struct obj *book2;
   ...
   if (invocation_pos (u.ux, u.uy) && !On_stairs (u.ux, u.uy))
      ...
      register boolean arti1_primed = FALSE,
            arti2_primed = FALSE,
            arti_cursed = FALSE;
      ...
      if (arti_cursed)
         ...
         {                          /* at least one artifact not prepared
                                 * properly */
...
STATIC_PTR int learn ()
   ...
   for (i = 0; i < MAXSPELL; i++)
      ...
      if (spl_book[i].sp_id == booktype)
         {
         if (book->spestudied >= rn1 (1, 10 - spl_book[i].sp_lev)) // GNU was 8-
            {
            pline ("This spellbook is too faint to be read anymore.");
            book->otyp = booktype = SPE_BLANK_PAPER;
            makeknown ((int) booktype);
            }
         else if (spl_book[i].sp_uses < 12 - spl_book[i].sp_lev/2) // GNU was 11- and added /2
            {
            Your ("knowledge of that spell is keener.");

//            spl_book[i].sp_uses += rn1 (1, 9 - spl_book[i].sp_lev);
//
//          GNU *** give more charges ***
//
            spl_book[i].sp_uses += 8 + rn1 (1, 15 - spl_book[i].sp_lev);
...
static int getspell ()
   ...
   if (spl_book[0].sp_id == NO_SPELL)
      ...
      for (i = 0; (i < maxs) && (i < 26); buf[++i] = 0)
         buf[i] = 'a' + i;
      for (i = 26; (i < maxs) && (i < 52); buf[++i] = 0)
         buf[i] = 'A' + i - 26;

      if (maxs == 1)
         Strcpy (lets, "a");
      else if (maxs < 27)
         Sprintf (lets, "a-%c", 'a' + maxs - 1);
      else if (maxs == 27)
         Sprintf (lets, "a-z A");
      else
         Sprintf (lets, "a-z A-%c", 'A' + maxs - 27);
      for (;;)
         {
...
boolean atme;
   ...
   if (confused ||
      ...
      if (Hallucination)
         ...
         pline ("The air around you crackles as you goof up.");
      return (0);
      }
   exercise (A_WIS, TRUE);
...
//
// GNU give a bit more spell info
//
      Sprintf (buf, "%c> %s  Uses:%d  Lev:%d",
               spellet (i),   spellname (i), 
               spelluses (i), spellev (i));
