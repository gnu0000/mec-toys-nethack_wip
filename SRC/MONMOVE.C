...
register struct monst *mtmp;
   ...
   else if (!IS_ROCK (here->typ))   /* no dig */
      return (FALSE);
...
register struct monst *mtmp;
   ...
   if (Warning && !rd && !mtmp->mpeaceful &&
      ...
      {
      /* Note: this assumes we only want to warn against the monster
       * to which the weapon does extra damage, as there is no
       * "monster which the weapon warns against" field. */
...
register struct monst *mtmp;
   {
   /* + Ettins are hard to surprise. + Nymphs, jabberwocks, and
    * leprechauns do not easily wake up.
    * 
    * Wake up if: in direct LOS                  AND within 10 squares
    * ND not stealthy or (mon is an ettin and 9/10)      AND (mon is
    * not a nymph, jabberwock, or leprechaun) or 1/50   AND Aggravate
    * or mon is (dog or human) or (1/7 and mon is not mimicing
    * furniture or object) */
   if (couldsee (mtmp->mx, mtmp->my) &&
...
int  *inrange,
   ...
   *nearby = *inrange && monnear (mtmp, mtmp->mux, mtmp->muy);
    ...
    * invisible, it sees the Elbereth at your real position, thus
    * never running into you by accident but possibly attacking the
    * spot where it guesses you are. */
...
/*   Pre-movement adjustments   */
...
#endif
      return (0);                 /* other frozen monsters can't do
                                 * anything */
      }
    ...
    * set_apparxy() call in m_move() doesn't suffice since the
    * variables inrange, etc. all depend on stuff set by
    * set_apparxy(). */
...
/*   Now the actual movement phase   */
   ...
   (is_wanderer (mdat) && !rn2 (4)) || (Conflict && !mtmp->iswiz) ||
      ...
      switch (tmp)
         ...
         case 0:                 /* no movement, but it can still
                                 * attack you */
         case 3:                 /* absolutely no movement */
            /* for pets, case 0 and 3 are equivalent */
            /* During hallucination, monster appearance should still
             * change - even if it doesn't move. */
            if (Hallucination)
               newsym (mtmp->mx, mtmp->my);
            break;
         case 1:                 /* monster moved */
            ...
            if (!nearby && ranged_attk (mdat))
               ...
               {
               /* a monster that's digesting you can move at the same
                * time -dlc */
...
register int after;
   ...
   xchar gx,
         ...
         niy,
         chcnt;
   int   chi;                    /* could be schar except for stupid
                                 * Sun-2 compiler */
   boolean likegold = 0,
         likegems = 0,
         likeobjs = 0,
   ...
   ptr = mtmp->data;              /* mintrap() can change mtmp->data
                                 * -dlc */
...
#define SQSRCHRADIUS   5
   ...
   look_for_obj:
         ...
         for (otmp = fobj; otmp; otmp = otmp->nobj)
            ...
            xx = otmp->ox;
            yy = otmp->oy;
            /* Nymphs take everything.  Most other creatures should
             * not pick up corpses except as a special case like in
   ...
   if (mmoved)
       ...
       * thinks it is.  In either case, attack this spot in
      ...
      /* The monster may attack another based on 1 of 2 conditions: 1
       * - It may be confused. 2 - It may mistake the monster for
       * your (displaced) image. Pets get taken care of above and
       * shouldn't reach this code. Conflict gets handled even
       * farther away (movemon()). */
      if ((info[chi] & ALLOW_M) ||
...
postmov:
   if (mmoved == 1)
      ...
      if (can_tunnel && mdig_tunnel (mtmp))
         return (2);              /* mon died (position already
                                 * updated) */
      ...
      if (OBJ_AT (mtmp->mx, mtmp->my) && mtmp->mcanmove)
         ...
         if ((!abstain || !rn2 (10)) &&
           (!*in_rooms (mtmp->mx, mtmp->my, SHOPBASE) || !rn2 (25)))
            {
            if (likeobjs)
               mpickstuff (mtmp, practical);
            if (likemagic)
               mpickstuff (mtmp, magical);
            if (likerock || likegems)
               mpickgems (mtmp);
#ifdef MUSE
            if (uses_items)
               mpickstuff (mtmp, (char *) 0);
#endif
            }
...
register struct monst *mtmp;
   ...
   /* If invis but not displaced, staying around gets you
    * 'discovered' */
