...
#include "hack.h"
 ...
 /* creating a new version which requires that the old one be
  * deletable. */
...
#define   NAMSZ   10
#define   DTHSZ   60
#define   PERSMAX    3              /* entries per name/uid per char.
                                 * allowed */
#define   POINTSMIN   1           /* must be > 0 */
#define   ENTRYMAX   100           /* must be >= 10 */

#if !defined(MICRO) && !defined(MAC)
#define   PERS_IS_UID              /* delete for PERSMAX per name; now
                                 * per uid */
#endif
struct toptenentry
   {
...
#define   HUP   if(!done_hup)
...
#ifdef TOS
   restore_colors ();           /* make sure the screen is black on
                                 * white */
#endif
    ...
    * seem right, so we have to report the death level in depth()
    * terms as well (which also seems reasonable since that's all the
    * player sees on the screen anyway) */
...
#ifdef LOGFILE                    /* used for debugging (who dies of
                                 * what, where) */
...
#  endif
   ...
   /* We would like to determine whether you're experienced.
    * However, the information collected here only tells about the
    * scores/roles that got into the topten (top 100?).  We should
    * maintain a .hacklog or something in his home directory. */
...
boolean fem;
   ...
   switch (plch)
      {
      case 'A':
         return PM_ARCHEOLOGIST;
      case 'B':
         return PM_BARBARIAN;
      case 'C':
         return (fem ? PM_CAVEWOMAN : PM_CAVEMAN);
      case 'E':
         return PM_ELF;
      case 'H':
         return PM_HEALER;
      ...
      case 'K':
         return PM_KNIGHT;
      case 'P':
         return (fem ? PM_PRIESTESS : PM_PRIEST);
      case 'R':
         return PM_ROGUE;
      ...
      case 'W':
         return PM_WIZARD;
      default:
         impossible ("What weird class is this? (%c)", plch);
