...
boolean flag;
   ...
   int   x2,
         y2;
   boolean horiz;
   boolean lava = levl[x][y].typ == LAVAPOOL;   /* assume initialized
                                                 * map */
...
struct entity
   ...
   struct permonst *edata;        /* must be non-zero for record to be
                                 * valid */
   int   ex,
         ey;
   };
   ...
   if (is_u (etmp))
      ...
      if (how == BURNING)
         ...
         if (!e_survives_at (etmp, etmp->ex, etmp->ey))
            ...
            /* otherwise on top of the drawbridge is the only viable
             * spot in the dungeon, so stay there */
            }
         }
      }
   else
...
struct entity *etmp;
   ...
   if (e_missed (etmp, FALSE))
      ...
      if (crm->typ == DRAWBRIDGE_DOWN)
         ...
         return;                 /* Note: Beyond this point, we know
                                 * we're  */
         }                          /* not at an opened drawbridge, since
                                 * all  */
      must_jump = TRUE;           /* *missable* creatures survive on
                                 * the     */
      }                             /* square, and all the unmissed ones
                                 * die.  */
   if (must_jump)
      {
      if (at_portcullis)
...
int   x,
   ...
   if (cansee (x, y))           /* change msgs if you are a w-walker
                                 * at portcullis */
...
int   x,
   ...
   do_entity (&(occupants[1]));
   spoteffects ();              /* if underwater, hero is now on
                                 * solid ground */
   (void) revive_nasty (x, y, NULL);
   delallobj (x, y);
   newsym (x, y);
...
int   x,
   ...
   int   x2,
         y2;
   boolean e_inview;
   struct entity *etmp1 = &(occupants[0]),
        *etmp2 = &(occupants[1]);
   ...
   set_entity (x2, y2, etmp2);  /* currently only automissers can be
                                 * here */
   if (etmp2->edata)
      {
      ...
      if (!automiss (etmp2))
         ...
         }                          /* nothing which is vulnerable can
                                 * survive this */
      }
