...
#ifdef DUMB                       /* stuff commented out in extern.h,
                                 * but needed here */
 ...
 */
#define BSIZE 20
static char pushq[BSIZE],
      saveq[BSIZE];
static NEARDATA int phead,
      ptail,
      shead,
      stail;
...
static char
      popch ()
   {
   /* If occupied, return '\0', letting tgetch know a character
    * should be read from the keyboard.  If the character read is not
    * the ABORT character (as checked in pcmain.c), that character
    * will be pushed back on the pushq. */
   if (occupation)
      return '\0';
   if (in_doagain)
...
int
      doextlist ()              /* here after #? - now list all
                                 * full-word commands */
   {
...
STATIC_PTR int
   ...
   if (can_breathe (uasmon))
      return dobreathe ();
   else if (attacktype (uasmon, AT_SPIT))
      return dospit ();
   else if (u.usym == S_NYMPH)
      return doremove ();
   else if (u.usym == S_UMBER)
      return doconfuse ();
   else if (is_were (uasmon))
      return dosummon ();
   else if (webmaker (uasmon))
      return dospinweb ();
   else if (is_hider (uasmon))
      return dohide ();
   ...
   else if (u.umonnum == PM_MIND_FLAYER)
      return domindblast ();
...
#ifdef WIZARD
STATIC_PTR int
      wiz_wish ()                 /* Unlimited wishes for debug mode by
                                 * Paul Polderman */
   {
   if (wizard)
      {
...
boolean final;
   ...
   if (Fast)
      you_are ((Fast & ~INTRINSIC) ? "very fast" : "fast");
   if (Stealth)
      you_are ("stealthy");
   if (Regeneration)
...
int
      xytod (x, y)              /* convert an x,y pair into a
                                 * direction code */
schar x,
      y;
   {
...
#ifdef WALKIES
void
      dtoxy (cc, dd)              /* convert a direction code into an
                                 * x,y pair */
coord *cc;
register int dd;
   {
...
int
      movecmd (sym)              /* also sets u.dz, but returns false
                                 * for <> */
char  sym;
   {
...
int   x,
   ...
   if (x == 0 && y == 0)        /* map click on player to "rest"
                                 * command */
      return '.';
...
STATIC_OVL char *
   ...
   multi = 0;
   flags.move = 1;
   flush_screen (1);              /* Flush screen buffer. Put the
                                 * cursor on the hero. */
