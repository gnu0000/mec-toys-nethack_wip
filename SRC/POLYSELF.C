...
void
   ...
   if (Polymorph_control)
      ...
      do
         ...
         /* Note:  humans are illegal as monsters, but an illegal
          * monster forces newman(), which is what we want if they
          * specified a human.... */
   ...
   /* The below polyok() fails either if everything is genocided, or
    * if we deliberately chose something illegal to force newman(). */
...
int   mntmp;
   ...
   if (u.umonnum == -1)
      ...
      {
      /* Monster to monster; restore human stats, to be immediately
       * changed to provide stats for the new monster */
      u.acurr = u.macurr;
      u.amax = u.mamax;
      flags.female = u.mfemale;
      }
   ...
   if (strongmonst (&mons[mntmp]))
      ABASE (A_STR) = AMAX (A_STR) = 118;

   if (resists_ston (uasmon) && Stoned)
   ...
   else if (is_golem (uasmon))
      u.mhmax = golemhp (mntmp);
   else
      {
      /* tmp = adj_lev(&mons[mntmp]); We can't do this, since there's
       * no such thing as an "experience level of you as a monster"
   ...
   if (!sticky && !u.uswallow && u.ustuck && sticks (uasmon))
      u.ustuck = 0;
   else if (sticky && !sticks (uasmon))
      uunstick ();
...
int   part;
   ...
   if (u.usym == S_CENTAUR || u.usym == S_UNICORN)
   ...
   if (u.usym == S_VORTEX || u.usym == S_ELEMENTAL)
...
int   damtype,
    ...
    * monster-specific slow/haste so there is no way to restore the
    * old velocity once they are back to human. */
