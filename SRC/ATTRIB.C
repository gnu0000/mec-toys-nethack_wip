...
//      7, 10, 10, 7, 7, 7
//
// GNU *** up starting strengthfor Archeologist, ok so its cheating ***
//
      14, 10, 10, 7, 7, 7
      },                             /* Archeologist */
      {
...
int   ndx,
      incr;
int   msgflg;                    /* positive => no message, zero =>
                                 * message, and */
   {                                /* negative => conditional (msg if
                                 * change made) */
   if (!incr)
      return FALSE;
...
void
      losestr (num)              /* may kill you; cause may be poison
                                 * or monster like 'a' */
register int num;
   {
...
boolean parameter;              /* So I can't think up of a good
                                 * name.  So sue me. --KAA */
   {
   register struct obj *otmp;
...
boolean inc_or_dec;
   ...
   if (abs (AEXE (i)) < AVAL)
       ...
       * 
       * Gain is harder at higher attribute values. 79% at "3" --> 0% at
       * "18" Loss is even at all levels (50%).
       * 
...
static void
   ...
   if (!(moves % 5))
      ...
      if (Wounded_legs || Fumbling || HStun)
         exercise (A_DEX, FALSE);
      }
   }
...
void
   ...
   if (moves >= next_check && !multi)
       ...
       * 
       * The effects of "exercise" and "abuse" wear off over time.  Even
       * if you *don't* get an increase/decrease, you lose some of
      ...
      for (i = 0; i < A_MAX; AEXE (i++) /= 2)
          ...
          * 
          * You don't *always* gain by exercising. [MRS 92/10/28 - Treat
          * Wisdom specially for balance.] */
...
static void
      init_align ()
   {                                /* called from newhp if u.ulevel is 0 */
   ...
   if (abil)
      ...
      for (; abil->ability; abil++)
         ...
         if (oldlevel < abil->ulevel && newlevel >= abil->ulevel)
            {
            /* Abilities gained at level 1 can never be lost via
             * level loss, only via means that remove _any_ sort of
             * ability.  A "gain" of such an ability from an outside
             * source is devoid of meaning, so we set FROMOUTSIDE to
             * avoid such gains. */
            if (abil->ulevel == 1)
