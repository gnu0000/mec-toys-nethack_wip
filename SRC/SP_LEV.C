...
xchar rtype,
    ...
    * random we are willing to make several try before we give it up. */
   do
      {
      xchar xborder,
            yborder;
...
struct mkroom *croom;
xchar walls;                    /* any of W_NORTH | W_SOUTH | W_EAST
                                 * | W_WEST (or W_ANY) */
   {
   xchar sx,
         sy;                    /* location of the secret door */
   int   count;
...
struct mkroom *croom;
   ...
   if (!class)
      ...
      /* if we can't get class for a specific monster type, it means,
       * that it's extinct, genocided, or unique, and shouldn't be
       * created. */
      if (!pm)
         return;
      }
...
struct mkroom *croom;
   ...
   if (!c)
      ...
      {
      /* The special levels are compiled with the default "text"
       * object class characters.  We must convert them to the
       * internal format. */
   ...
   switch (o->curse_state)
      ...
      case 3:
         ...
         break;                 /* CURSED */
      default:
         break;                 /* Otherwise it's random and we're
                                 * happy with what mkobj gave us! */
      }

   /* corpsenm is "empty" if -1, random if -2, otherwise specific */
   if (o->corpsenm == -2)
      otmp->corpsenm = rndmonnum ();
   else if (o->corpsenm != -1)
...
struct mkroom *croom;
   ...
   /* Is the alignment random ? If so, it's an 80% chance that the
    * altar will be co-aligned.
    * 
   ...
   if (a->shrine == -11)
      a->shrine = rn2 (1);        /* handle random case */
...
#endif
      /* MRS - 07/04/91 - This is temporary but should result in
       * proper filling of shops, etc. DLC - this can fail if
       * corridors are added to this room at a later point.
       * Currently no good way to fix this. */
...
int   typ;
   ...
   int   i;
      ...
      /* shuffle 3 alignments; can't use sp_lev_shuffle() on
       * aligntyp's */
      i = rn2 (3);
      atmp = ralign[2];
      ralign[2] = ralign[i];
...
FILE *fd;
   ...
   /* Now that we have loaded all the rooms, search the subrooms and
    * create the links. */
...
FILE *fd;
   ...
   while (numpart--)
      ...
      /* If any CROSSWALLs are found, must change to ROOM after
       * REGION's are laid out.  CROSSWALLS are used to specify
       * "invisible" boundaries where DOOR syms look bad or aren't
       * desirable. */
      has_bounds = FALSE;
      ...
      if (init_lev.init_present && xsize <= 1 && ysize <= 1)
         ...
         for (y = ystart; y < ystart + ysize; y++)
            for (x = xstart; x < xstart + xsize; x++)
                ...
                * lev_comp.y saves it as type char. Since schar !=
                * char all the time we must make this exception or
                * hack through lev_comp.y to fix. */
               ...
               if (levl[x][y].typ == SDOOR || IS_DOOR (levl[x][y].typ))
                  ...
                  /* If there is a wall to the left that connects to
                   * a (secret) door, then it is horizontal.  This
                   * does not allow (secret) doors to be corners of
                   * rooms. */
      ...
      /* now that we have rooms _and_ associated doors, fill the
       * rooms */
   ...
   while (nwalk--)
       ...
       * walkfrom() is odd.  But we must also take into account what
   ...
   /* If there's a significant portion of maze unused by the special
    * level, we don't want it empty.
    * 
    * Makes the number of traps, monsters, etc. proportional to the size
    * of the maze. */
