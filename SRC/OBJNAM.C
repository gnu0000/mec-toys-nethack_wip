...
register struct obj *obj;
   {
#ifdef LINT                       /* lint may handle static decl poorly
                                 * -- static char bufr[]; */
   char  bufr[BUFSZ];
   ...
   switch (obj->oclass)
      ...
      case AMULET_CLASS:
         ...
         else if (nn)           /* should be true for the Amulet of
                                 * Yendor */
            Strcpy (buf, actualn);
         else if (un)
...
register struct obj *obj;
   ...
   /* when we have to add something at the start of prefix instead of
    * the end (Strcat is used on the end) */
   register char *bp = xname (obj);

   /* When using xname, we want "poisoned arrow", and when using
    * doname, we want "poisoned +0 arrow".  This kludge is about the
    * only way to do it, at least until someone overhauls xname() and
    * doname(), combining both into one function taking a parameter. */
   ...
   if (obj->bknown &&
      ...
      {
      /* allow 'blessed clear potion' if we don't know it's holy
       * water; always allow "uncursed potion of water" */
      if (obj->cursed)
          ...
          * totally identified the item, so "uncursed" is
          * unneccesary, because an identified object not described
          * as "blessed" or "cursed" must be uncursed.
          * 
...
const char *oldstr;
   ...
   *(spot + 1) = 0;
...
#ifdef TUTTI_FRUTTI
   /* mouse/mice,louse/lice (not a monster, but possible in food
    * names) */
   ...
   /* Ends in y preceded by consonant (note: also "qu") change to
    * "ies" */
   if (*spot == 'y' &&
       (!index (vowels, *(spot - 1))))
      {
...
register char *bp;
    ...
    * who names their fruit "katana" and wishes for a katana gets a
    * real one.  But, we have to keep around the old buf since in the
    * meantime we have deleted "empty", "+6", etc... */
#endif
   char  let;
   char *un,
        *dn,
...
*/
    ...
    * potions called whisky scrolls labeled "QWERTY" egg fortune
    * cookies very heavy iron ball named hoei wand of wishing elven
    * cloak */
   ...
   /* Find corpse type using "of" (figurine of an orc, tin of orc
    * meat) Don't check if it's a wand or spellbook. (avoid
    * "wand/finger of death" confusion). */
   if (!strstri (bp, "wand ")
   ...
   if (strncmp (bp, "samurai sword", 13))   /* not the "samurai"
                                           * monster! */
      if (strncmp (bp, "wizard lock", 11))   /* not the "wizard"
                                              * monster! */
...
#endif
   /* NOTE: Gold pieces are handled as objects nowadays, and
    * therefore this section should probably be reconsidered as well
    * as the entire gold/money concept.  Maybe we want to add other
    * monetary units as well in the future. (TH) */
...
# endif
      if (!BSTRCMP (bp, p - 5, "altar"))
         ...
         if (!strncmpi (bp, "chaotic ", 8))
            ...
            al = A_NONE;
         else                    /* -1 - A_CHAOTIC, 0 - A_NEUTRAL, 1 -
                                 * A_LAWFUL */
...
#ifdef TUTTI_FRUTTI
   /* Note: not strncmpi.  2 fruits, one capital, one not, is
    * possible. */
   for (f = ffruit; f; f = f->nextf)
      {
      char *f1 = f->fname,
           *f2 = makeplural (f->fname);
...
any:
   if (!let)
      let = wrpsym[rn2 ((int) sizeof (wrpsym))];
typfnd:
   if (typ)
      let = objects[typ].oc_class;
   ...
   if (mntmp > -1)
      switch (typ)
         ...
         case FIGURINE:
            ...
            break;
         case EGG:
            if (lays_eggs (&mons[mntmp]) || mntmp == PM_KILLER_BEE)
               otmp->corpsenm = mntmp;
            break;
         case STATUE:
            otmp->corpsenm = mntmp;
   ...
   /* set blessed/cursed -- setting the fields directly is safe since
    * weight() is called below and addinv() will take care of luck */
   if (iscursed)
      {
      curse (otmp);
      }
