 ...
 */
   switch (mtmp->data->mlet)
      ...
      case S_GIANT:
         if (rn2 (2))
            (void) mongets (mtmp, (ptr != &mons[PM_ETTIN]) ?
                            BOULDER : CLUB);
         break;
      case S_HUMAN:
         if (is_mercenary (ptr))
      ...
      case S_HUMANOID:
         ...
         break;
# ifdef KOPS
      case S_KOP:              /* create Keystone Kops with cream
                                 * pies to throw. As suggested by
                                 * KAA.      [MRS] */
         if (!rn2 (4))
...
# endif
      ...
      case S_DEMON:
         ...
         /* prevent djinnis and mail daemons from leaving objects
          * when they vanish */
         if (!is_demon (ptr))
            break;
         /* fall thru */
...
# endif
      ...
      case S_QUANTMECH:
         if (!rn2 (20))
            ...
            /* actually, whether this is a corpse or a live cat
             * shouldn't really be decided until the box is opened... */
...
#ifdef ARMY                       /* ordinary soldiers rarely have
                                 * access to magic (or gold :-) */
...
register int x,
   ...
   if (ptr)
      {
      /* if you are to make a specific monster and it has already
       * been genocided, return */
      if (ptr->geno & G_GENOD)
         return ((struct monst *) 0);
      }
   else
      {
      /* make a random (common) monster that can survive here. (the
       * special levels ask for random monsters at specific
       * positions, causing mass drowning on the medusa level, for
       * instance.) */
   ...
   *mtmp = zeromonst;           /* clear all entries in structure */
   ...
   if (is_golem (ptr))
      ...
      {
      /* "special" fixed hp monster the hit points are encoded in the
       * mlevel in a somewhat strange way to fit in the 50..127
       * positive range of a signed character above the 1..49 that
       * indicate "normal" monster levels */
   ...
   if (ptr == &mons[PM_CHAMELEON])
      {
      /* If you're protected with a ring, don't create any
       * shape-changing chameleons -dgk */
   ...
   m_initinv (mtmp);              /* add on a few special items incl.
                                 * more armor */
#ifdef MUSE
   m_dowear (mtmp, TRUE);
#endif
...
int   x,
      y;
struct monst *mtmp;              /* existing monster being moved, if
                                 * any */
struct permonst *mdat;
   {
   struct monst *mtmp2;
    ...
    * can't go on top of the player or any existing monster. however,
    * occasionally we are relocating engravings or objects, which
    * could be colocated and thus get restricted a bit too much. oh
    * well. */
   if (x == u.ux && y == u.uy)
      return 0;
   if ((mtmp2 = m_at (x, y)) && mtmp != mtmp2)
      return 0;
   ...
   if (mdat)
      ...
      if (IS_POOL (levl[x][y].typ))
         if (mdat == &playermon &&
            ...
            return (is_flyer (mdat) || is_swimmer (mdat));
...
struct monst *mtmp;
   {
   register int x = xupstair,
         y = yupstair,
         trycount;

   /* if the wiz teleports away to heal, try the up staircase, to
    * block the player's escaping before he's healed */
...
void
      rloc_shk (mtmp)           /* to be used when teleporting a
                                 * shopkeeper */
struct monst *mtmp;
   {
   register int x,
...
static boolean
      cmavail ()                 /* return TRUE if "common" monsters
                                 * can be generated */
   {
   struct permonst *ptr;
...
register struct permonst *ptr;
   {
   static NEARDATA long oldmoves = 0L;   /* != 1, starting value of
                                        * moves */
...
#ifdef MULDGN
   if (u.uz.dnum == quest_dnum && (ptr = qt_montype ()))
      return (ptr);
#endif
   if (oldmoves != moves)
      {                             /* must recalculate accept */
      oldmoves = moves;
...
int   spc;
   ...
   register int first,
         ...
         num = 0;
   int   maxmlev,
         mask = (G_GENOD | G_EXTINCT | G_NOGEN | G_UNIQ) & ~spc;
...
int
      adj_lev (ptr)              /* adjust strength of monsters based
                                 * on u.uz and u.ulevel */
register struct permonst *ptr;
   {
   int   tmp,
         tmp2;
...
struct permonst *
      grow_up (mtmp, victim)     /* mon mtmp "grows up" to a bigger
                                 * version. */
register struct monst *mtmp;
...
register struct permonst *ptr;
   ...
   /* Last case:  a chance of a co-aligned monster being hostile.
    * This chance is greater if the player has strayed
    * (u.ualign.record negative) or the monster is not strongly
    * aligned. */
...
struct monst *mtmp;
   ...
   else                          /* not coaligned and therefore
                                 * hostile */
      mtmp->malign = abs (mal);
   }
...
register struct monst *mtmp;
   ...
   if (OBJ_AT (mx, my))
      ...
      ap_type = M_AP_FURNITURE;
      appear = S_altar;
      /* We won't bother with beehives, morgues, barracks, throne
       * rooms since they shouldn't contain too many mimics anyway... */
      }
   else if (rt >= SHOPBASE)
      {
