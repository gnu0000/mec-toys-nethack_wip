...
void
   ...
   /* Note:  these initializers don't do anything except guarantee
   ...
   (void) encumber_msg ();        /* in case they auto-picked up
                                 * something */
   ...
   for (;;)
      ...
      UnloadAllSegments ();     /* Marks non-resident segments as
                                 * purgeable */
#endif
      get_nh_event ();
      ...
      if (flags.move)
         ...
         if (flags.time)
            flags.botl = 1;
         /* One possible result of prayer is healing.  Whether or not
          * you get healed depends on your current hit points. If you
          * are allowed to regenerate during the prayer, the
...
void
...
#ifdef MAC
   /* This _is_ the right place for this - maybe we will have to
    * split display_gamewindows into create_gamewindows and
    * show_gamewindows to get rid of this ifdef... */
   ...
   /* The mac port is not DEPENDENT on the order of these displays,
    * but it looks a lot better this way... */
...
void
   ...
   vision_reset ();              /* set up internals for level (after
                                 * mklev) */

   flags.botlx = 1;

   /* Move the monster from under you or else makedog() will fail
    * when it calls makemon(). - ucsfcgl!kneller */
   if (MON_AT (u.ux, u.uy))
