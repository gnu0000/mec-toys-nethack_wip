...
struct obj *
      select_rwep (mtmp)        /* select a ranged weapon for the
                                 * monster */
register struct monst *mtmp;
   ...
   if (throws_rocks (mtmp->data))   /* ...boulders for giants */
      Oselect (BOULDER);
   /* other than these two specific cases, always select the most
    * potent ranged weapon to hand. */
   for (i = 0; i < SIZE (rwep); i++)
      ...
      /* propellor = obj, propellor to use propellor = &zeroobj,
       * doesn't need a propellor propellor = 0, needed one and
       * didn't have one */
      if (propellor != 0)
         {
         /* Note: cannot use m_carrying for loadstones, since it will
          * always select the first object of a type, and maybe the
          * monster is carrying two but only the first is
          * unthrowable. */
         if (rwep[i] != LOADSTONE)
            {
#ifdef MUSE
...
static NEARDATA const struct hwep
   {
   short otyp,
         big;
   }     hwep[] =

   {
      ...
      {
      DWARVISH_SHORT_SWORD, 0
      },
      {
      SHORT_SWORD, 0
      },
      {
      ORCISH_SHORT_SWORD, 0
      },
      {
      MACE, 0
      },
...
struct obj *
      select_hwep (mtmp)        /* select a hand to hand weapon for
                                 * the monster */
register struct monst *mtmp;
   {
   register struct obj *otmp;
...
register struct monst *mon;
    ...
    * this is OK since the player can't see it. Note that if there is
    * no change, setting the check to NEED_WEAPON is harmless.
    * Possible problem: big monster with big cursed weapon gets
    * polymorphed into little monster.  But it's not quite clear how
