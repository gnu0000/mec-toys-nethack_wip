...
int
   ...
   while (new_wormno < MAX_NUM_WORMS)
      ...
      if (!wheads[new_wormno])
         return new_wormno;     /* found an empty wtails[] slot at
                                 * new_wormno */
      new_wormno++;
      }
...
int   wseg_count;
   ...
   register struct wseg *seg,
        *new_tail = create_worm_tail (wseg_count);
...
/*  if (!wnum) return; /* bullet proofing */
   ...
   /* Place a segment at the old worm head.  The head has already
    * moved. */
   seg = wheads[wnum];
   place_worm_seg (worm, seg->wx, seg->wy);
   newsym (seg->wx, seg->wy);     /* display the new segment */

   /* Create a new dummy segment head and place it at the end of the
    * list. */
   new_seg = newseg ();
   new_seg->wx = worm->mx;
   new_seg->wy = worm->my;
...
struct obj *weap;
   ...
   /* Split the worm.  The tail for the new worm is the old worm's
    * tail. The tail for the old worm is the segment that follows
    * "curr", and "curr" becomes the dummy segment under the new
    * head. */
   ...
   *new_worm = *worm;           /* make a copy of the old worm */
   ...
   /* Calculate the mhp on the new_worm for the (lower) monster
    * level. */
   new_worm->mhpmax = new_worm->mhp = d ((int) new_worm->m_lev, 8);

   /* Calculate the mhp on the old worm for the (lower) monster
    * level. */
   if (worm->m_lev > 3)
      {
...
/*  if (!wnum) return; /* bullet proofing */
   ...
   while (curr)
      ...
      if (tryct < 50)
         ...
         {                          /* Oops.  Truncate because there was */
