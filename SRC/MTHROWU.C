...
int
      thitu (tlev, dam, obj, name)   /* u is hit by sth, but not a
                                     * monster */
register int tlev,
      dam;
struct obj *obj;
...
register struct obj *obj;
    ...
    * inventory).  Remove it.  We can do this in 1 line, but it's
    * highly dependent on the fact that we know splitobj() places it
    * immediately after obj. */
...
#ifdef MUSE
   singleobj->owornmask = 0;     /* threw one of multiple weapons in
                                 * hand? */
#endif
   ...
   /* Note: drop_throw may destroy singleobj.  Since obj must be
    * destroyed early to avoid the dagger bug, anyone who modifies
    * this code should be careful not to use either one after it's
    * been freed. */
   if (sym)
      tmp_at (DISP_FLASH, obj_to_glyph (singleobj));
   while (range-- > 0)
      {                             /* Actually the loop is always exited
                                 * by break */
      boolean vis;
      ...
      if (MON_AT (bhitpos.x, bhitpos.y))
         ...
         if (find_mac (mtmp) + 8 + singleobj->spe <= rnd (20))
            ...
            if (ismimic)
               seemimic (mtmp);
            if (!vis)
               pline ("It is hit%s", exclam (damage));
            else
               hit (distant_name (singleobj, xname),
                    mtmp, exclam (damage));
            ...
            if (objects[singleobj->otyp].oc_material == SILVER
               ...
               {
               if (vis)
                  pline ("The silver sears %s's flesh!",
                         mon_nam (mtmp));
               else
      ...
      if (bhitpos.x == u.ux && bhitpos.y == u.uy)
         ...
         if (hitu && singleobj->opoisoned)
            /* it's safe to call xname twice because it's the same
             * object both times... */
         ...
         if (hitu && (singleobj->otyp == CREAM_PIE ||
            ...
            if (singleobj->otyp == CREAM_PIE)
               ...
               if (!Blind)
                  ...
                  pline ("There's something sticky all over your %s.", body_part (FACE));
               }
            else
               {                    /* venom in the eyes */
...
register struct monst *mtmp;
   ...
   if (lined_up (mtmp))
      ...
      if ((otmp = select_rwep (mtmp)))
         {
         /* If you are coming toward the monster, the monster should
          * try to soften you up with missiles.  If you are going
          * away, you are probably hurt or running.  Give chase, but
          * if you are getting too far away, throw. */
         x = mtmp->mx;
         y = mtmp->my;
         if (!URETREATING (x, y) ||
...
register xchar ax,
   ...
   if ((!tbx || !tby || abs (tbx) == abs (tby))   /* straight line or
                                                 * diagonal */
...
boolean
lined_up (mtmp)                 /* is mtmp in position to use ranged
                                 * attack? */
register struct monst *mtmp;
   {
