...
void
   ...
   if (u.uluck && moves % (u.uhave.amulet || u.ugangr ? 300 : 600) == 0)
      {
      /* Cursed luckstones stop bad luck from timing out; blessed
       * luckstones stop good luck from timing out; normal luckstones
       * stop both; neither is stopped if you don't have a luckstone.
       * Luck is based at 0 usually, +1 if a full moon and -1 on
       * Friday 13th */
   ...
   if (u.uinvulnerable)
      return;                    /* things past this point could kill
                                 * you */
   if (Stoned)
      stoned_dialogue ();
   if (Vomiting)
                     ...
                     switch (rn2 (4))
                        ...
                        case 1:
                           if (ACCESSIBLE (levl[u.ux][u.uy].typ))
                              {     /* not POOL or STONE */
                              if (Hallucination)
                                 pline ("A rock bites your %s.", body_part (FOOT));
                              else
...
register struct obj *otmp;
   ...
   if (egg_age > 200L)
      {                             /* very old egg - it's dead */
      otmp->corpsenm = -1;
      return;
      }
   else if (egg_age <= 150L)
      {                             /* too young to hatch */
      return;
      }
...
void
      hatch_eggs ()              /* hatch any eggs that have been too
                                 * long in pack */
   {
   register struct obj *otmp,
        *otmp2;
