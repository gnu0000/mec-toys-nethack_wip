...
static NEARDATA struct
   {
   struct obj *piece;           /* the thing being eaten, or last
                                 * thing that was partially eaten,
                                 * unless that thing was a tin, which
                                 * uses the tin structure above */
...
/*ARGSUSED*/
static void
      choke (food)              /* To a full belly all food is bad.
                                 * (It.) */
register struct obj *food;
   ...
   if (!rn2 (20) || Breathless)
      ...
      /* Note all "killer"s below read "Choked on %s" on the high
       * score list & tombstone.  So plan accordingly. */
      if (food)
         {
#ifdef POLYSELF
...
static void
      recalc_wt ()              /* modify object wt. depending on
...
void
      reset_eat ()              /* called when eating interrupted by
                                 * an event */
   {
   /* we only set a flag here - the actual reset process is done
    * after the round is spent eating. */
...
static void
   ...
   /* Do not set canchoke to FALSE; if we continue eating the same
    * object we need to know if canchoke was set when they started
    * eating it the previous time.  And if we don't continue eating
    * the same object canchoke always gets recalculated anyway. */
   stop_occupation ();
   }

STATIC_PTR
int
      eatfood ()                 /* called each move during eating
                                 * process */
   {
...
static void
      cpostfx (pm)              /* called after completely consuming
                                 * a corpse */
register int pm;
   {
   register int tmp = 0;
   ...
   switch (pm)
      ...
      case PM_SMALL_MIMIC:
         ...
         if (u.usym == S_HUMAN)
            ...
            u.usym = 0;           /* hack! no monster sym 0; use for
                                 * gold */
            newsym (u.ux, u.uy);
            }
         break;
      ...
      default:
            ...
            /* prevent polymorph abuse by killing/eating your
             * offspring */
             ...
             * monster can give more than one, pick one to try to
             * give from among all it can give.
             * 
             * If a monster can give 4 intrinsics then you have a 1/1 *
             * 1/2 * 2/3 * 3/4 = 1/4 chance of getting the first, a
             * 1/2 * 2/3 * 3/4 = 1/4 chance of getting the second, a
             * 1/3 * 3/4 = 1/4 chance of getting the third, and a 1/4
             * chance of getting the fourth.
             * 
             * And now a proof by induction: it works for 1 intrinsic (1
             * in 1 of getting it) for 2 you have a 1 in 2 chance of
             * getting the second, otherwise you keep the first for 3
             * you have a 1 in 3 chance of getting the third,
             * otherwise you keep the first or the second for n+1 you
             * have a 1 in n+1 chance of getting the (n+1)st,
             * otherwise you keep the previous one. Elliott
             * Kleinrock, October 5, 1990 */
            ...
            for (i = 1; i <= LAST_PROP; i++)
               ...
               if (intrinsic_possible (i, ptr))
                  {
                  count++;
                  /* a 1 in count chance of replacing the old one
                   * with this one, and a count-1 in count chance of
                   * keeping the old one.  (note that 1 in 1 and 0 in
                   * 1 are what we want for the first one */
                  if (!rn2 (count))
                     {
#ifdef DEBUG
...
STATIC_PTR
int
      opentin ()                 /* called during each move whilst
                                 * opening a tin */
   {
   register int r;
...
int
      Hear_again ()              /* called when waking up after
                                 * fainting */
   {
   flags.soundok = 1;
   return 0;
...
static int
      eatcorpse (otmp)           /* called when a corpse is selected
                                 * as food */
register struct obj *otmp;
   {
   ...
   if (otmp->corpsenm != PM_LIZARD)
      {
#ifndef LINT                    /* problem if more than 320K moves
                                 * before try to eat */
...
register struct obj *otmp;
   ...
   if (otmp->otyp == CORPSE)
      ...
      if (!victual.piece && victual.eating)
         do_reset_eat ();
      if (victual.eating == FALSE)
         return;                 /* died and lifesaved */
      }
...
static void
      fprefx (otmp)              /* called on "first bite" of
                                 * (non-corpse) food */
...
static void
      fpostfx (otmp)              /* called after consuming
                                 * (non-corpse) food */
...
int
      doeat ()                    /* generic "eat" command funtion (see
                                 * cmd.c) */
   {
   register struct obj *otmp;
    ...
    * mails, players who polymorph back to human in the middle of
    * their metallic meal, etc.... */
   if (!is_edible (otmp))
      {
      You ("cannot eat that!");
      return 0;
   ...
   if (otmp == victual.piece)
      {
      /* If they weren't able to choke, they don't suddenly become
       * able to choke just because they were interrupted.  On the
       * other hand, if they were able to choke before, if they lost
       * food it's possible they shouldn't be able to choke now. */
      if (u.uhs != SATIATED)
   ...
   /* Now we need to calculate delay and nutritional info. The base
    * nutrition calculated here and in eatcorpse() accounts for
    * normal vs. rotten food.  The reqtime and nutrit values are then
    * adjusted in accordance with the amount of food left. */
   if (otmp->otyp == CORPSE)
      {
      if (eatcorpse (otmp))
         return (1);
    ...
    * due to this method. TODO: add in a "remainder" value to be
    * given at the end of the meal. */
...
void
      gethungry ()              /* as time goes by - called by
                                 * moveloop() and domove() */
   {
   if (u.uinvulnerable)
   ...
   if (moves % 2)
      ...
      if ((HRegeneration & (~W_ART)) &&
          (HRegeneration != W_WEP || !uwep->oartifact))
         u.uhunger--;
...
void
      morehungry (num)           /* called after vomiting and after
...
void
      lesshungry (num)           /* called after eating (and after
...
register int num;
   ...
   if (u.uhunger >= 2000)
      ...
      {
      /* Have lesshungry() report when you're nearly full so all
       * eating warns when you're about to choke. */
      if (u.uhunger >= 1500)
         {
...
void
      reset_faint ()              /* call when a faint must be
...
void
      newuhs (incr)              /* compute and comment on your (new?)
                                 * hunger status */
boolean incr;
   {
   register int newhs,
...
const char *verb;
int   corpsecheck;              /* 0, no check, 1, corpses, 2,
                                 * tinnable corpses */
   {
   register struct obj *otmp;
   char  qbuf[QBUFSZ];
...
#endif
   /* Is there some food (probably a heavy corpse) here on the
    * ground? */
...
#ifdef POLYSELF
   /* We cannot use ALL_CLASSES since that causes getobj() to skip
    * its "ugly checks" and we need to check for inedible items. */
