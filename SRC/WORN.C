      ...
      if (wp->w_mask & mask)
         ...
         *(wp->w_obj) = obj;
         if (obj)
            ...
            /* prevent getting intrinsics from wielding potions,
             * etc... */
...
boolean creation;
   ...
   /* Note the restrictions here are the same as in dowear in
    * do_wear.c except for the additional restriction on
    * intelligence.  (Players are always intelligent, even if
    * polymorphed). */
   if (verysmall (mon->data) || nohands (mon->data))
      return;
