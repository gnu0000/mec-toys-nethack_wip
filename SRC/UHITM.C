...
struct monst *mon;
   ...
   if (MON_AT (mm.x, mm.y) || mon->mhp <= 1)
      return (struct monst *) 0;
   /* may have been extinguished for population control */
   if (mon->data->geno & G_EXTINCT)
      return ((struct monst *) 0);
   m2 = newmonst (0);
   *m2 = *mon;                    /* copy condition of old monster */
   ...
   /* since shopkeepers and guards will only be cloned if they've
    * been polymorphed away from their original forms, the clone
    * doesn't have room for the extra information.  we also don't
    * want two shopkeepers around for the same shop. similarly,
    * clones of named monsters don't have room for the name, so we
    * just make the clone unnamed instead of bothering to create a
    * clone with room and copying over the name from the right place
   ...
   if (mon->mtame)
       ...
       * however, tamedog will not re-tame a tame dog, so m2 must be
       * made non-tame to get initialized properly. */
      m2->mtame = 0;
...
/*   Adjust vs. (and possibly modify) monster state.      */
   ...
   if (is_orc (mtmp->data) && pl_character[0] == 'E')
      tmp++;
...
register struct monst *mtmp;
    ...
    * Protection is provided as long as player is not: blind,
    * confused, hallucinating or stunned. changes by wwp 5/16/85 More
    * changes 12/90, -dkh-. if its tame and safepet, (and protected
    * 07/92) then we assume that you're not trying to attack.
    * Instead, you'll usually just swap places if this is a movement
    * command */
   /* Intelligent chaotic weapons (Stormbringer) want blood */
   if (is_safepet (mtmp) &&
       (!uwep || uwep->oartifact != ART_STORMBRINGER))
      {
      /* there are some additional considerations: this won't work if
       * in a shop or Punished or you miss a random roll or if you
       * can walk thru walls and your pet cannot (KAA) or if your pet
       * is a long worm (unless someone does better). there's also a
       * chance of displacing a "frozen" monster. sleeping monsters
       * might magically walk in their sleep. */
   ...
   if (special_case (mtmp))
      return (TRUE);

#ifdef POLYSELF
   if (u.umonnum >= 0)
      {                             /* certain "pacifist" monsters don't
                                 * attack */
      set_uasmon ();
      if (noattacks (uasmon))
         {
...
static boolean
      known_hitum (mon, mhit)     /* returns TRUE if monster still
                                 * lives */
...
register int mhit;
   ...
   if (!mhit)
      ...
      if ((malive = hmon (mon, uwep, 0)) == TRUE)
         ...
         /* If no damage was done (Vorpal Blade and not on head) do
          * not cut the worm.  We lost the information long ago, so
...
static boolean
      hitum (mon, tmp)           /* returns TRUE if monster still
                                 * lives */
struct monst *mon;
int   tmp;
   {
...
register int thrown;
    ...
    * following order: 1) Know what we're attacking with, and print
    * special hittxt for unusual cases. 2a) Know whether we did
    * damage (depends on 1) 2b) Know if it's poisoned (depends on 1)
    * 2c) Know whether we get a normal damage bonus or not (depends
    * on 1) 3a) Know what the value of the damage bonus is (depends
    * on 2c) 3b) Know how much poison damage was taken (depends on
    * 2b) and if the poison instant-killed it 4) Know if it was
    * killed (requires knowing 3a, 3b) except by instant- kill poison
    * 5) Print hit message (depends on 1 and 4) 6a) Print poison
    * message (must be done after 5) #if 0 6b) Rust weapon (must be
    * done after 5) #endif 7) Possibly kill monster (must be done
    * after 6a, 6b) 8) Instant-kill from poison (can happen anywhere
    * between 5 and 9) 9) Hands not glowing (must be done after 7 and
    * 8) The major problem is that since we don't want a "hit"
    * message when the monster dies, we have to know how much damage
    * it did _before_ outputting a hit message, but any messages
    * associated with the damage don't come out until _after_
    * outputting a hit message. */
 ...
 */
   ...
   if (!hittxt && !destroyed)
      ...
      if (thrown)
         ...
         You ("hit %s%s", mon_nam (mon), canseemon (mon)
              ? exclam (tmp) : ".");
      }
...
register struct attack *mattk;
   ...
   switch (mattk->adtyp)
      ...
      case AD_FIRE:
         if (!Blind)
            pline ("%s is on fire!", Monnam (mdef));
...
# endif
      ...
      case AD_SITM:
         if (mdef->minvent)
            ...
            /* Without MUSE we can only change a monster's AC by
             * stealing armor with the "unarmored soldier" kludge.
             * With it there are many monsters which wear armor, and
             * all can be stripped. */
            if (
#ifndef MUSE
                is_mercenary (pd) &&
...
register struct attack *mattk;
    ...
    * would be necessary if done accurately.)  Instead, we
    * arbitrarily kill the monster immediately for AD_DGST and we
    * regurgitate them after exactly 1 round of attack otherwise.
    * -KAA */
...
register int tmp;
   ...
   for (i = 0; i < NATTK; i++)
      ...
      switch (mattk->aatyp)
         ...
         case AT_WEAP:
      use_weapon:
            /* Certain monsters don't use weapons when encountered as
             * enemies, but players who polymorph into them have
             * hands or claws and thus should be able to use weapons.
             * This shouldn't prohibit the use of most special
             * abilities, either. */
            /* Potential problem: if the monster gets multiple weapon
             * attacks, we currently allow the player to get each of
             * these as a weapon attack.  Is this really desirable? */
            if (uwep)
               tmp += hitval (uwep, mon->data);
            dhit = (tmp > (dieroll = rnd (20)) || u.uswallow);
            /* Enemy dead, before any special abilities used */
            if (!known_hitum (mon, dhit))
               return 0;
            ...
            break;
         case AT_CLAW:
            if (i == 0 && uwep && !cantwield (uasmon))
               goto use_weapon;
# ifdef SEDUCE
            /* succubi/incubi are humanoid, but their _second_ attack
             * is AT_CLAW, not their first... */
...
# endif
         ...
         case AT_HUGS:
            /* automatic if prev two attacks succeed, or if already
             * grabbed in a previous attack */
            dhit = 1;
            wakeup (mon);
            if (mon->data == &mons[PM_SHADE])
         ...
         case AT_MAGC:
            /* No check for uwep; if wielding nothing we want to do
             * the normal 1-2 points bare hand damage... */
      ...
      if (uasmon == &playermon)
         break;                 /* No extra attacks if no longer a
                                 * monster */
      if (multi < 0)
         break;                 /* If paralyzed while attacking, i.e.
                                 * floating eye */
      }
   return ((boolean) (nsum != 0));
   }
