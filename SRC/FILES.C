...
# ifdef VMS
char  lock[PL_NSIZ + 17] = "1lock";   /* long enough for
                                     * _uid+name+.99;1 */
...
char  SAVEF[SAVESIZE];           /* holds relative path of save file
                                 * from playground */
...
#else
# ifdef VMS                       /* essential to have punctuation, to
                                 * avoid logical names */
   char  tmp[BUFSIZ];
...
# if defined(VMS) && !defined(SECURE)
   /* Re-protect bones file with world:read+write+execute+delete
    * access. umask() doesn't seem very reliable; also, vaxcrtl won't
    * let us set delete access without write access, which is what's
    * really wanted. Can't simply create it with the desired
    * protection because creat ANDs the mask with the user's default
    * protection, which usually denies some or all access to world. */
   (void) chmod (bones, FCMASK | 007);   /* allow other users full
                                        * access */
# endif                          /* VMS && !SECURE */
#endif                          /* MICRO */
...
#  endif
#  if defined(VMS) && !defined(SECURE)
   /* Make sure the save file is owned by the current process.
    * That's the default for non-privileged users, but for priv'd
    * users the file will be owned by the directory's owner instead
    * of the user. */
...
static int lockfd;              /* for lock_file() to pass to
                                 * unlock_file() */
...
#ifdef VMS                       /* for unlock_file, use the unlink()
                                 * routine in vmsunix.c */
# ifdef unlink
#  undef unlink
# endif
...
#define fopenp fopen
   ...
   const char *filename;
      ...
      /* "filename" is an environment variable, so it should hang
       * around */
      if (filename)
         {
#ifdef UNIX
         if (access (filename, 4) == -1)
            ...
            /* nasty sneaky attempt to read file through NetHack's
             * setuid permissions -- this is the only place a file
             * name may be wholly under the player's control */
...
# ifdef MFLOPPY
         ...
         {
         /* The following ifdef is NOT in the wrong place.  For now,
          * we accept and silently ignore RAMDISK */
#  ifndef AMIGA
...
# endif
      ...
      /* We should try to get this data from a rsrc, in the profile
       * file the user double-clicked...  This data should be saved
       * with the save file in the resource fork, AND be saveable in
       * "stationery" */
...
# pragma unused(dir)
      ...
      if (fd >= 0)
         {
# ifdef VMS                       /* must be stream-lf to use
                                 * UPDATE_RECORD_IN_PLACE */
         if (!file_is_stmlf (fd))
            {
            raw_printf (        /* note: assume VMS dir has trailing
                                 * punct */
...
# if defined(VMS) && !defined(SECURE)
         /* Re-protect RECORD with world:read+write+execute+delete
          * access. */
         (void) chmod (RECORD, FCMASK | 007);   /* allow everyone full
                                                 * access */
# endif                          /* VMS && !SECURE */
         }
      else
