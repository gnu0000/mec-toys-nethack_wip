...
void
      o_unleash (otmp)           /* otmp is about to be destroyed or
                                 * stolen */
...
void
      m_unleash (mtmp)           /* mtmp is about to die, or become
                                 * untame */
register struct monst *mtmp;
   {
   register struct obj *otmp;
...
#ifdef OVLB
struct obj *
      get_mleash (mtmp)           /* assuming mtmp->mleashed has been
                                 * checked */
register struct monst *mtmp;
   {
   register struct obj *otmp;
...
boolean waslit,
   ...
   if (rockit)
      ...
      if ((mtmp = m_at (x, y)) != 0)   /* make sure crucial monsters
                                        * survive */
   ...
   if (dig_down)
      ...
      if (On_stairs (u.ux, u.uy))
         ...
         if (u.ux == xdnladder || u.ux == xupladder)
            ...
            pline ("The stairs are too hard to dig in.");
         return (0);
         }
...
int   x,
   ...
   register int x1,
         y1;
   int   lo_x = max (1, x - 1),
         hi_x = min (x + 1, COLNO - 1),
         lo_y = max (0, y - 1),
         hi_y = min (y + 1, ROWNO - 1);
   int   pool_cnt = 0,
         moat_cnt = 0,
         lava_cnt = 0;
...
int   ttyp;
   ...
   if (ttyp == PIT)
       ...
       * the hero does NOT fall down is treated here.  the case where
       * the hero does fall down is treated in goto_level(). */
      if (u.ustuck || wont_fall)
         ...
         if (!next_to_u ())
            ...
            /* earlier checks must ensure that the destination level
             * exists and is in the present dungeon. */
...
boolean pit_only;
   ...
   if (ttmp && (ttmp->ttyp == MAGIC_PORTAL || nohole))
      ...
      if (ttmp && (ttmp->ttyp == PIT || ttmp->ttyp == SPIKED_PIT))
         ...
         {
         /* digging makes a hole, but the boulder immediately fills
          * it.  Final outcome:  no hole, no boulder. */
...
struct obj *obj;
   ...
   if (mtmp->msleep)
      ...
      if (!rn2 (4))
         tmp = 120;
      /* Note: floating eyes cannot use their abilities while
       * invisible, but Medusa and umber hulks can. */
      if (vis)
...
boolean
snuff_candle (otmp)              /* call in drop, throw, and put in
                                 * box, etc. */
register struct obj *otmp;
   {
...
int
   ...
   if (!obj || (obj != uwep && !wield_tool (obj)))
      return 0;
...
/* this will have to change if more monsters can poly */
   ...
   if ((can = mksobj (TIN, FALSE, FALSE)) != 0)
      ...
      can->owt = weight (can);
      can->known = 1;
      can->spe = -1;              /* Mark tinned tins. No spinach
                                 * allowed... */
      if (carried (corpse))
         {
         if (corpse->unpaid)
...
struct obj *obj;
   ...
   if (obj && obj->cursed)
      ...
      switch (rn2 (6))
         ...
         case 0:
            make_sick (Sick ? Sick / 4 + 1L : (long) rn1 (ACURR (A_CON), 20), TRUE);
            Strcpy (buf, xname (obj));
            u.usick_cause = (const char *) buf;
            break;
         case 1:
            make_blinded (Blinded + (long) rnd (100), TRUE);
            break;
         case 2:
            if (!Confusion)
               You ("suddenly feel %s.",
            ...
            break;
         case 3:
            make_stunned (HStun + (long) rnd (100), TRUE);
            break;
         case 4:
            (void) adjattrib (rn2 (6), -1, FALSE);
            break;
         case 5:
            make_hallucinated (HHallucination + (long) rnd (100),
                               TRUE, 0L);
            break;
         }
      return;
