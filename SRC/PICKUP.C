...
long  resplit;                    /* non-zero => shift the quantities */
   ...
   if (obj_block->nobj != obj_chip)
      ...
      {                             /* 1st object should be reduced to
                                 * 'resplit' */
      ...
      {                             /* 2nd obj should be merged back into
                                 * 1st, then destroyed */
...
boolean here,
   ...
   ilets[iletct] = '\0';        /* terminate ilets so that index()
                                 * will work */
   while (otmp)
      {
...
int   all;                       /* all >= 0 => yes/no; < 0 => -count */
   ...
   if (all)
      {
      if (flags.pickup)
         Strcpy (oclasses, flags.pickup_types);
      }
   else
      {
   ...
   for (obj = level.objects[u.ux][u.uy]; obj; obj = obj2)
      ...
      if ((pick = pickup_object (obj, objx)) < 0)
         break;
      pick_count += pick;
      }
   ...
   /* see whether there's anything else here, after auto-pickup is
    * done */
   if (all && flags.pickup)
      check_here (pick_count > 0);
   }
...
struct obj *obj,
   ...
   /* in case of auto-pickup, where we haven't had a chance to look
    * at it yet; affects docall(SCR_SCARE_MONSTER) */
   if (!Blind)
      obj->dknown = 1;
   ...
   if (obj == uchain)
      ...
      if (obj->blessed)
         ...
         if (!(objects[SCR_SCARE_MONSTER].oc_name_known) &&
            ...
            docall (obj);
         useupf (obj);
         return 1;              /* tried to pick something up and
                                 * failed, but don't want to
                                 * terminate pickup loop yet   */
         }
      }
      ...
      if (obj->quan > 1L)
         ...
         if (qq)
            ...
            if (objx)
               ...
               if (objx->otyp == SCR_SCARE_MONSTER)
                  objx->spe = 0;
               }
            goto lift_some;
            }
...
register struct obj *otmp;
   ...
   if (*u.ushops && costly_spot (u.ux, u.uy) &&
       otmp != uball)           /* don't charge for this - kd,
                                 * 1/17/90 */
...
int   depthin;
   {
   /* odds: 1/1, 2/2, 3/4, 4/8, 5/16, 6/32, 7/64, 8/128, 9/128,
    * 10/128,... */
...
register struct obj *obj;
   ...
   if (!current_container)
      ...
      {
      /* Prohibit Amulets in containers; if you allow it, monsters
       * can't steal them.  It also becomes a pain to check to see if
       * someone has the Amulet.  Ditto for the Candelabrum, the Bell
       * and the Book. */
   ...
   if (obj->otyp == ICE_BOX || Is_box (obj))
      {
      /* xname() uses a static result array.  Save obj's name before
       * current_container's name is computed.  Don't use the result
       * of strcpy() within You() --- the order of evaluation of the
   ...
   if (is_gold)
      {                             /* look for other gold within the
                                 * container */
      for (gold = current_container->cobj; gold; gold = gold->nobj)
         if (gold->otyp == GOLD_PIECE)
            break;
      }
   else
      gold = 0;
   ...
   if (is_gold)
      ...
      if (gold)
         dealloc_obj (obj);
      bot ();                    /* update character's gold piece
                                 * count immediately */
      }
...
register int held;
   ...
   /* Count the number of contained objects. Sometimes toss objects
    * if */
   /* a cursed magic bag.                      */
   prev = (struct obj *) 0;
