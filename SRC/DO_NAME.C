...
register int ininv;
    ...
    * (e.g. Excalibur from prayer). In this case the object will
    * retain its current name. */
   if (obj->oartifact || exist_artifact (obj->otyp, buf))
      return obj;

   otmp2 = newobj (lth);
   *otmp2 = *obj;                 /* the cobj pointer is copied to
                                 * otmp2 */
   otmp2->onamelth = lth;
   ...
   if (obj->owornmask)
      {
      /* Note: dying by burning in Hell causes problems if you try
       * doing this when owornmask isn't set. */
   ...
   if (ininv)
      {
      /* do freeinv(obj); etc. by hand in order to preserve the
       * position of this object in the inventory */
      if (obj == invent)
         invent = otmp2;
      else
   ...
   dealloc_obj (obj);           /* let us hope nobody else saved a
                                 * pointer */
   return otmp2;
   }
...
register struct obj *obj;
   ...
   if (!*buf)
      {
      if (*str1)                 /* had name, so possibly remove from
                                 * disco[] */
...
const char *adjective;
   ...
   if (mtmp->ispriest || mtmp->isminion)
      ...
      if (article == 3 && !strncmp (name, "the ", 4))
         name += 4;
      return name;
      }
   ...
   if (mtmp->isshk)
      ...
      if (mtmp->data == &mons[PM_SHOPKEEPER] && !mtmp->minvis)
         return (buf);
      /* For normal shopkeepers, just 'Asidonhopo'. For unusual ones,
       * 'Asidonhopo the invisible shopkeeper' or 'Asidonhopo the
       * blue dragon'. */
      Strcat (buf, " ");
      }
   if (force_the ||
...
const char *
      rndmonnam ()
   {                                /* Random name of monster type, if
                                 * hallucinating */
   int   name;
   ...
   if (name >= PM_ARCHEOLOGIST)
      return bogusmons[name - PM_ARCHEOLOGIST];
