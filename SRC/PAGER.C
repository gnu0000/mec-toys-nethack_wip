...
char *buf;
   ...
   if (u.ux == x && u.uy == y && canseeself ())
      ...
      {
      /* all locations when swallowed other than the hero are the
       * monster */
...
boolean user_typed_name;
   ...
   if (*inp)
      {
      /* adjust the input to remove "named " and convert to lower
       * case */
      ...
      /* If the object is named, then the name is the alternate
       * description; otherwise, the result of makesingular() applied
       * to the name is. This isn't strictly optimal, but named
       * objects of interest to the user will usually be found under
       * their name, rather than under their object type, so looking
       * for a singular form is pointless. */
      ...
      while (fgets (buf, BUFSZ, fp))
         {
         if (*buf == '.')
            break;              /* we passed last entry without
                                 * success */
...
static int
      do_look (quick)
boolean quick;                    /* use cursor && don't search for
                                 * "more info" */
   {
   char  out_str[BUFSZ],
         look_buf[BUFSZ];
   ...
   /* The user typed one letter, or we're identifying from the
    * screen. */
   do
      {
      ...
      /* Check all the possibilities, saving all explanations in a
       * buffer. When all have been checked then the string is
       * printed. */
      ...
      /* Special case: if identifying from the screen, and we're
       * swallowed, and looking at something other than our own
       * symbol, then just say "the interior of a monster". */
      ...
      /* If we are looking at the screen, follow multiple
       * posibilities or an ambigious explanation by something more
       * detailed. */
      if (from_screen)
         {
...
#ifdef WIZARD
# ifdef PORT_HELP
# define WIZHLP_SLOT 12           /* assumed to be next to last by code
                                 * below */
   "k.  List of wizard-mode commands.",
# else
# define WIZHLP_SLOT 11           /* assumed to be next to last by code
                                 * below */
