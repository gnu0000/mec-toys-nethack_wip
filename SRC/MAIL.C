...
extern char *viz_rmin,
     *viz_rmax;                 /* line-of-sight limits (vision.c) */
...
coord *startp;
   ...
   /* If blind and not telepathic, then it doesn't matter what we
    * pick --- the hero is not going to see it anyway.  So pick a
    * nearby position. */
   if (Blind && !Telepat)
      {
   ...
   /* Arrive at an up or down stairwell if it is in line of sight
    * from the hero. */
   ...
   /* Try to pick a location out of sight next to the farthest
    * position away from the hero.  If this fails, try again, just
    * picking the farthest position that could be seen.  What we
    * really ought to be doing is finding a path from a stairwell...
    * 
    * The arrays viz_rmin[] and viz_rmax[] are set even when blind.
...
struct monst *md;
register int tx,
      ty;                       /* destination of mail daemon */
   {
   struct monst *mon;           /* displaced monster */
   register int dx,
         dy;                    /* direction counters */
   int   fx = md->mx,
         fy = md->my;           /* current location */
   int   nfx = fx,
         nfy = fy,              /* new location */
         d1,
         d2;                    /* shortest distances */
   ...
   while (1)
      ...
      if ((mon = m_at (fx, fy)) != 0)   /* save monster at this
                                        * position */
      ...
      /* Remove md from the dungeon.  Restore original mon, if
       * necessary. */
      if (mon)
         {
   ...
   /* Check for a monster at our stopping position (this is possible,
    * but very unlikely).  If one exists, then have the md leave in
    * disgust. */
   if ((mon = m_at (fx, fy)) != 0)
      {
...
struct mail_info *info;
   ...
   if (!(md = makemon (&mons[PM_MAIL_DAEMON], start.x, start.y)))
      goto give_up;
...
go_back:
   ...
   mongone (md);
   /* deliver some classes of messages even if no daemon ever shows
    * up */
give_up:
...
void
   ...
   while (broadcasts > 0)
      {                             /* process all trapped broadcasts
                                 * [until] */
      broadcasts--;
      if ((brdcst = parse_next_broadcast ()) != 0)
         {
         newmail (brdcst);
         break;                 /* only handle one real message at a
                                 * time */
         }
      }
   }
...
struct obj *otmp;
   {
#  ifdef SHELL                    /* can't access mail reader without
                                 * spawning subprocess */
   const char *txt,
        *cmd;
   char *p,
