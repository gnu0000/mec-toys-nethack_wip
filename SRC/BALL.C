...
#define BCPOS_DIFFER   0           /* ball & chain at different
                                 * positions */
...
void
   ...
   {
   if (u.uswallow)
      return;                    /* ball&chain not placed while
                                 * swallowed */
...
int   already_blind;
   ...
   /* Since we can still see, remove the ball&chain and get the glyph
    * that would be beneath them.  Then put the ball&chain back.
    * This is pretty disgusting, but it will work. */
...
xchar ballx,
   ...
   if (Blind)
      {
      /* The hero is blind.  Time to work hard.  The ball and chain
       * that are attached to the hero are very special.  The hero
       * knows that they are attached, so when they move, the hero
       * knows that they aren't at the last position remembered.
       * This is complicated by the fact that the hero can "feel" the
       * surrounding locations at any time, hence, making one or both
       * of them show up again. So, we have to keep track of which is
       * felt at any one time and act accordingly. */
      if (!before)
         {
      ...
      {
      /* The hero is not blind.  To make this work correctly, we need
       * to pick up the ball and chain before the hero moves, then
       * put them in their new positions after the hero moves. */
      if (before)
         {
         if (!control)
            {
            /* Neither ball nor chain is moving, so remember which
             * was on top until !before.  Use the variable u.bc_order
...
xchar x,
   ...
   if (x != u.ux || y != u.uy)
      ...
      vision_full_recalc = 1;     /* hero has moved, recalculate vision
                                 * later */
...
void
    ...
    * 
    * a) the character is wielding it, or b) the character has both
    * hands available to hold it (i.e. is not wielding any weapon),
    * or c) (perhaps) it falls forward out of his non-weapon hand */
