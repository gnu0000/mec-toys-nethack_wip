...
static void
      dump_qtlist ()              /* dump the character msg list to
                                 * check appearance */
   {
   struct qtmsg *msg;
   long  size;
...
void
   ...
   /* Read in the number of classes, then the ID's & offsets for each
    * header. */
   ...
   /* Now construct the message lists for quick reference later on
    * when we are actually paging the messages out. */
...
static struct qt_matrix
   ...
   short mtyp1,
         mtyp2;                 /* monster types for enemies 0 ==
                                 * random */
   char  msym1,
         msym2;                 /* monster classes for enemies */
...
static unsigned
   ...
   switch (pl_character[0])
      ...
      case 'C':
         return (2);
      case 'E':
         return ((unsigned) (3 + flags.female));
      case 'H':
         return (5);
      case 'K':
         return (6);
...
static const char *
      intermed ()                 /* return your intermediate target
                                 * string */
   {
...
static const char *
      guardname ()              /* return your class leader's guard
                                 * monster name */
   {
...
static const char *
      homebase ()                 /* return your class leader's
                                 * location */
   {
...
char  c;
   ...
   switch (c)
      ...
      case 'c':
         ...
         break;
      case 'r':
         str = rank_of (u.ulevel, pl_character[0], flags.female);
         break;
      case 'R':
         str = rank_of (MIN_QUEST_LEVEL, pl_character[0],
                        flags.female);
         break;
      case 's':
      ...
      case 'i':
         ...
         break;
      case 'o':
         str = the (artiname (qt_matrix[class_index ()].artinum));
         break;
      case 'n':
         str = neminame ();
         break;
