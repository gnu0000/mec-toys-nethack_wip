      ...
      for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
         for (obj = mtmp->minvent; obj; obj = obj->nobj)
            if ((temp = o_in (obj, oclass)) != 0)
               ...
               temp->oy = mtmp->my;
               map_object (temp, 1);
               break;           /* skip rest of this monster's
                                 * inventory */
               }
      newsym (u.ux, u.uy);
      if (sobj)
         Your ("nose tingles and you smell %s.", what);
      else
         You ("sense %s.", what);
      display_nhwindow (WIN_MAP, TRUE);
      exercise (A_WIS, TRUE);
...
int   class;                    /* an object class, 0 for all */
   ...
   u.uinwater = 0;
   /* If we are mapping all objects, map only the top object of a
    * pile or the first object in a monster's inventory.  Otherwise,
    * go looking for a matching object class and display the first
    * one encountered at each location. */
   for (x = 1; x < COLNO; x++)
      for (y = 0; y < ROWNO; y++)
   ...
   for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
      ...
      /* Allow a mimic to override the detected objects it is
       * carrying. */
      if (is_cursed && mtmp->m_ap_type == M_AP_OBJECT &&
          (!class || class == objects[mtmp->mappearance].oc_class))
         {
         ...
         temp.otyp = mtmp->mappearance;   /* needed for
                                           * obj_to_glyph() */
         temp.ox = mtmp->mx;
         temp.oy = mtmp->my;
   ...
   display_nhwindow (WIN_MAP, TRUE);
   /* What are we going to do when the hero does an object detect
    * while blind and the detected object covers a known pool? */
...
struct obj *obj;
   ...
   if (oops && (obj->spe > 0))
      ...
      switch (rnd (obj->oartifact ? 4 : 5))
         {
         case 1:
            pline ("%s is too much to comprehend!", The (bname));
            break;
         case 2:
            pline ("%s confuses you!", The (bname));
         ...
         case 3:
            ...
            break;
         case 4:
            pline ("%s zaps your mind!", The (bname));
            make_hallucinated (HHallucination + rnd (100), FALSE, 0L);
            break;
         case 5:
            pline ("%s explodes!", The (bname));
            useup (obj);
      ...
      if (!obj->spe)
         ...
         switch (rnd (6))
            ...
            case 2:
               ...
               break;
            case 3:
               pline ("The crystal pulses with sinister %s light!", hcolor ());
               break;
            case 4:
...
register int x,
   ...
   /* if not a room and we don't remember an object or trap there,
    * map it */
   if (lev->typ != ROOM &&
...
int
      openit ()                 /* returns number of things found and
                                 * opened */
   {
   int   num = 0;
