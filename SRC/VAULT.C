...
fnd:
      ...
      if (!(guard = makemon (&mons[PM_GUARD], x, y)))
         return;
      guard->isgd = 1;
      guard->mpeaceful = 1;
      set_malign (guard);
      ...
      if (IS_WALL (levl[x][y].typ))
         ...
         {                          /* the initial guard location is a
                                 * dug door */
         int   vlt = EGD (guard)->vroom;
         xchar lowx = rooms[vlt].lx,
               hix = rooms[vlt].hx;
         xchar lowy = rooms[vlt].ly,
               hiy = rooms[vlt].hy;
...
register struct monst *grd;
   ...
   TRUE : FALSE;
   boolean disappear_msg_seen = FALSE,
         semi_dead = (grd->mhp <= 0);
   ...
   if (abs (egrd->ogx - grd->mx) > 1 ||
      ...
      return (-1);              /* teleported guard - treat as
                                 * monster */
   if (egrd->fcend == 1)
      {
      if (u_in_vault &&
...
long
   ...
   /* unknown gold stuck inside statues may cause some
    * consternation... */
...
#ifdef SOUNDS
boolean
gd_sound ()                       /* prevent "You hear footsteps.."
                                 * when inappropriate */
   {
   register struct monst *grd = findgd ();
