...
const struct shclass shtypes[] =    {
    ...
    * random implementor name (along with candle shops having random
    * shopkeepers) */
      {"rare books", SPBOOK_CLASS, 3, D_SHOP,
      {   {90, SPBOOK_CLASS   },    {10, SCROLL_CLASS   },    {0, 0   }   }, shkbooks   },
   /* Shops below this point are "unique".  That is they must all
    * have a probability of zero.  They are only created via the
...
static int
      shkinit (shp, sroom)        /* create a new shopkeeper in the
                                 * given room */
const struct shclass *shp;
struct mkroom *sroom;
   {
   ...
   if (sroom->irregular)
      ...
      if (isok (sx - 1, sy) &&
          levl[sx - 1][sy].roomno == rmno && !levl[sx - 1][sy].edge)
         sx--;
      else if (isok (sx + 1, sy) &&
          levl[sx + 1][sy].roomno == rmno && !levl[sx + 1][sy].edge)
         sx++;
      else if (isok (sx, sy - 1) &&
          levl[sx][sy - 1].roomno == rmno && !levl[sx][sy - 1].edge)
         sy--;
      else if (isok (sx, sy + 1) &&
          levl[sx][sy + 1].roomno == rmno && !levl[sx][sy + 1].edge)
         sx++;
...
# endif
   ...
   if (MON_AT (sx, sy))
      rloc (m_at (sx, sy));     /* insurance */

   /* now initialize the shopkeeper monster structure */
   if (!(shk = makemon (&mons[PM_SHOPKEEPER], sx, sy)))
      return (-1);
...
void stock_room (shp_indx, sroom)
int  shp_indx;
register struct mkroom *sroom;
   {
   /* Someday soon we'll dispatch on the shdist field of shclass to
    * do different placements in this routine. Currently it only
    * supports shop-style placement (all squares except a row nearest
    * the first door get objects). */
...
//         mkshobj_at (shp, sx, sy);
//
//  GNU *** allow 30% chance of >1 item on a square ***
//
         do
            {
            mkshobj_at (shp, sx, sy);
    ...
    * monsters will sit on top of objects and not the other way
    * around. */
