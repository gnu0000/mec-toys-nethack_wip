...
#ifdef OPTION_LISTS_ONLY        /* want option lists for external
                                 * program */
#include "config.h"
#include "objclass.h"
#include "flag.h"
...
# else                          /* systems that support multiple
                                 * terminals, many monochrome */
      {
...
void
   ...
   /* Set defaults for some options depending on what we can detect
    * about the environment's capabilities. This has to be done after
    * the global initialization above and before reading
    * user-specific initialization via config file/environment
    * variable below. */
...
char *op;
   ...
   for (sp = flags.inv_order, num = strlen (buf); *sp; sp++)
      if (!index (buf, *sp))
         {
         buf[num++] = *sp;
         buf[num] = '\0';        /* explicitly terminate for next
                                 * index() */
         }
...
goodfruit:
      ...
      if (!initial)
         (void) fruitadd (pl_fruit);
      /* If initial, then initoptions is allowed to do it instead of
       * here (initoptions always has to do it even if there's no
       * fruit option at all.  Also, we don't want people setting
       * multiple fruits in their options.) */
      return;
      }
#endif
   /* graphics:string */
   ...
   if (!strncmpi (opts, "objects", 7))
      ...
      /* Override the default object class symbols.  The first object
       * in the object class is the "random object".  I don't want to
       * use 0 as an object class, so the "random object" is
   ...
   if (!strncmpi (opts, "pickup_types", 4))
      ...
      boolean badopt = FALSE,
            compat = (strlen (opts) <= 6);

      flags.pickup_types[0] = '\0';   /* all */
      if (!(op = string_for_opt (opts, compat)))
         {
         /* for backwards compatibility, "pickup" without a value is
          * a synonym for boolean autopickup, and pickup_types gets
          * reset to "all"                */
   ...
   if (!strncmpi (opts, "disclose", 4))
      ...
      if (!(op = string_for_opt (opts, TRUE)))
         {
         /* for backwards compatibility, "disclose" without a value
          * means all (was inventory and attributes, the only things
          * available then), but negated it means "none" (note "none"
          * contains none of "iavkg") */
         if (negated)
            Strcpy (flags.end_disclose, "none");
         return;
         }
      num = 0;
   ...
   /* OK, if we still haven't recognized the option, check the
...
# endif
         ...
         if ((boolopt[i].addr) == &flags.time
            ...
            {
            /* All corridor squares seen via night vision or candles
             * & lamps change.  Update them by calling newsym() on
             * them.  Don't do this if we are initializing the
             * options --- the vision system isn't set up yet. */
...
char *str;
   ...
   /* if not user-specified, then it's a fruit name for a fruit on a
    * bones level... */
