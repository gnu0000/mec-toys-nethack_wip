...
int
   ...
   if (!hu)
      ...
      (void) dosave0 ();
# ifdef VMS
      /* don't call exit when already within an exit handler; that
       * would cancel any other pending user-mode handlers */
      if (!exiting)
# endif
         terminate (1);
      }
...
# endif
   ...
   /* While copying level files around, zero out u.uz to keep parts
    * of the restore code from completely initializing all in-core
    * data structures, since all we're doing is copying. This also
    * avoids at least one nasty core dump. */
   uz_save = u.uz;
   ...
   for (ltmp = (xchar) 1; ltmp <= maxledgerno (); ltmp++)
      ...
      if (ofd < 0)
         {
         if (!hu)
            pline ("Cannot read level %d.", ltmp);
         (void) close (fd);
         (void) delete_savefile ();
         if (!hu)
            done (TRICKED);
...
void
   ...
   /* When checkpointing is on, the full state needs to be written on
    * each checkpoint.  When checkpointing is off, only the pid needs
    * to be in the level.0 file, so it does not need to be constantly
    * rewritten.  When checkpointing is turned off during a game,
    * however, the file has to be rewritten once to truncate it and
    * avoid restoring from outdated information.
    * 
    * Restricting havestate to this routine means that an additional
    * noop pid rewriting will take place on the first "checkpoint"
    * after the game is started or restored, if checkpointing is off. */
      ...
      for (y = 0; y < ROWNO; y++)
         ...
         for (x = 0; x < COLNO; x++)
            ...
            if (prm->glyph == rgrm->glyph
               ...
               {
               /* the run has been broken, write out run-length
                * encoding */
         writeout:
               bwrite (fd, (genericptr_t) & match, sizeof (uchar));
               bwrite (fd, (genericptr_t) rgrm, sizeof (struct rm));
               /* start encoding again. we have at least 1 rm in the
                * next run, viz. this one. */
               match = 1;
               rgrm = prm;
               }
            }
...
# else
   char  buf[BUFSIZ];           /* this is system interaction,
                                 * therefore BUFSIZ instead of
                                 * NetHack's BUFSZ */
   int   nfrom,
         nto,
         fdfrom,
