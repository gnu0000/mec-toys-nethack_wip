...
register struct obj *otmp;
   ...
   if (wake)
      ...
      if (mtmp->mhp > 0)
         ...
         seemimic (mtmp);        /* might unblock if mimicing a
                                 * boulder/door */
      }
   return 0;
   }
...
register struct obj *obj;
   ...
   if (obj->spe &&
       !(obj->otyp == WAN_CANCELLATION ||   /* can't cancel
                                           * cancellation */
...
struct obj *obj;
int   ochance,
      achance;                    /* percent chance for ordinary
                                 * objects, artifacts */
   {
...
struct obj *obj;
   ...
   switch (poly_zapped)
      ...
      case MINERAL:
         ...
         break;
      case 0:
         /* there is no flesh type, but all food is type 0, so we use
          * it */
...
#endif
int
      bhito (obj, otmp)           /* object obj was hit by the effect
                                 * of wand otmp */
register struct obj *obj,
     *otmp;                       /* returns TRUE if sth was done */
   {
   register int res = 1;
   struct obj *otmp2;
   ...
   if (obj == uball)
      ...
      switch (otmp->otyp)
         ...
         case SPE_POLYMORPH:
            ...
            if (index (charged_objs, otmp2->oclass))
               otmp2->spe = obj->spe;
...
register struct obj *obj;        /* returns nonzero of something was
                                 * hit */
...
register struct obj *obj;
   ...
   if (objects[obj->otyp].oc_dir == IMMEDIATE)
      ...
      switch (obj->otyp)
         ...
         case WAN_DIGGING:
         case SPE_DIG:
            /* Original effect (approximately): from CORR: dig until
             * we pierce a wall from ROOM: piece wall and dig until
             * we reach an ACCESSIBLE place. Currently: dig for
             * digdepth positions; also down on request of Lennart
             * Augustsson. */
               {
               register struct rm *room;
               register int digdepth;
               register boolean shopdoor,
                     shopwall;
...
register const char *force;     /* usually either "." or "!" */
   ...
   if (!cansee (bhitpos.x, bhitpos.y) || !flags.verbose)
      ...
      pline ("%s hits %s%s", The (str), mon_nam (mtmp), force);
   }
                     ...
                     while (1)
                        ...
                        switch (rn2 (5))
                           ...
                           case 2:
                              ...
                              break;
                           case 3:
                              if (!rust_dmg (uarmg, "gloves", 0, FALSE))
                                 continue;
                              break;
                           case 4:
                              if (!rust_dmg (uarmf, "boots", 0, FALSE))
                                 continue;
                              break;
                           }
                        break;  /* Out of while loop */
...
register struct obj *obj;
   ...
   if (otx == 0 && oty == 0)
      {
      ;                          /* fell through a trapdoor; no update
                                 * of old loc needed */
      }
   else
      {
...
void
      fracture_rock (obj)        /* fractured by pick-axe or wand of
                                 * striking */
