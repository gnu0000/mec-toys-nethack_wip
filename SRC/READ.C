...
#ifndef NO_SIGNAL
   scroll->in_use = TRUE;        /* scroll, not spellbook, now being
                                 * read */
#endif
...
int   curse_bless;
   ...
   if (obj->oclass == WAND_CLASS)
      ...
      if (obj->spe > rn2 (7) || obj->spe <= -5)
         ...
         /* oartifact: if a touch-sensitive artifact ring is ever
          * created the above will need to be revised  */
         }
      }
   else
      {
...
boolean howmuch;
   ...
   /* Make sure that what was seen is restored correctly.  To do
    * this, we need to go blind for an instant --- turn off the
    * display, then restart it.  All this work is needed to correctly
    * handle walls which are stone on one side and wall on the other.
    * Turning off the seen bit above will make the wall revert to
    * stone,  but there are cases where we don't want this to happen.
    * The easiest thing to do is to run it through the vision system
    * again, which is always correct. */
...
register struct obj *sobj;
   ...
   switch (sobj->otyp)
      ...
      case SCR_MAIL:
         ...
         /* note to the puzzled: the game Larn actually sends you
          * junk mail if you win! */
         else
            readmail (sobj);
         break;
#endif
...
do_it:
   /* No-op in water - can only see the adjacent squares and that's
    * it! */
   if (Underwater || Is_waterlevel (&u.uz))
      return;
   /* If we are darkening the room and the hero is punished but not
    * blind, then we have to pick up and replace the ball and chain
    * so that we don't remember them if they are out of sight. */
   ...
   /* If we are not blind, then force a redraw on all positions in
    * sight by temporarily blinding the hero.  The vision
    * recalculation will correctly update all previously seen
    * positions *and* correctly set the waslit bit [could be messed
    * up from above]. */
   if (!Blind)
      {
      vision_recalc (2);
      ...
      for (i = 0; i < NUMMONS; i++)
         ...
         if (mons[i].mlet == class)
            ...
            if (&mons[i] == player_mon () || ((mons[i].geno & G_GENO)
                                      && !(mons[i].geno & G_GENOD)))
               {
               /* This check must be first since player monsters
                * might have G_GENOD or !G_GENO. */
               ...
               /* for simplicity (and fairness) let's avoid alignment
                * changes here... */
#ifdef POLYSELF
               if (i == u.umonnum)
                  rehumanize ();
...
deadmeat:
   if (Hallucination)
      ...
      Strcpy (buf, ptr->mname);  /* make sure we have standard
                                 * singular */
   if (how & REALLY)
      {
...
#ifdef POLYSELF
         /* Polymorphed characters will die as soon as they're
          * rehumanized. */
         if (u.umonnum >= 0)
            You ("feel dead inside.");
         else
