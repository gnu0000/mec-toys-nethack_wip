...
int
   ...
   if (obj == uwep)
      ...
      if (obj->quan > 1L)
         ...
         if (uwep)
            return (1);           /* unwielded, died, rewielded */
         }
      }
   else if (obj->quan > 1L)
...
register boolean broken;
   ...
   else if (costly_spot (u.ux, u.uy) && costly_spot (x, y))
      {
      if (obj->unpaid)
         subfrombill (obj, shkp);
...
register struct obj *obj;
    ...
    * 
    * Dex still gives a bonus, but strength does not. Polymorphed
    * players lacking attacks may still throw. There's a base -1 to
    * hit. No bonuses for fleeing or stunned targets (they don't
    * dodge melee blows as readily, but dodging arrows is hard
    * anyway). Not affected by traps, etc. Certain items which don't
    * in themselves do damage ignore tmp. Distance and monster size
   ...
   if (uarmg && uwep && -objects[uwep->otyp].w_propellor == WP_BOW)
      ...
      switch (uarmg->otyp)
         ...
         case GAUNTLETS_OF_DEXTERITY:
            break;
         default:
            impossible ("Unknown type of gloves (%d)",
                        uarmg->otyp);
         }
      }
   ...
   if (is_orc (mon->data) && pl_character[0] == 'E')
      tmp++;
...
register boolean loose;           /* if not loose, obj is in fobj chain */
   ...
   /* it is currently assumed that 'loose' is co-extensive with
    * 'thrown'.  if this changes, an explicit 'thrown' arg must be
    * added to breaks() to ensure proper treatment of shop objs. */
   if (loose)
      {
...
struct obj *obj;
   ...
   if (u.dz)
      ...
      if (flooreffects (obj, u.ux, u.uy, "fall"))
         return (1);
      if (u.dz > 0)
         pline ("The gold hits the %s.", surface (u.ux, u.uy));
