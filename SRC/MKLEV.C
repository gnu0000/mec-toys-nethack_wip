...
*/
   return (vx == vy) ? 0 : -1;
#else
   register const struct mkroom *x,
        *y;
...
static void finddpos (cc, xl, yl, xh, yh)
coord *cc;
xchar xl,      yl,      xh,      yh;
   {
   register xchar x,         y;
...
boolean is_room;
   ...
   croom->rtype = rtype;
   croom->doorct = 0;
   /* if we're not making a vault, doorindex will still be 0 if we
    * are, we'll have problems adding niches to the previous room
...
register int type;
   ...
   if (!IS_WALL (levl[x][y].typ))   /* avoid SDOORs on already made
                                     * doors */
      type = DOOR;
   levl[x][y].typ = type;
   if (type == DOOR)
...
//          depth (&u.uz) < depth (&medusa_level) &&
//          rn2 (depth (&u.uz)) < 3)
// *** GNU - increase chance of shops ***
//
   if (depth (&u.uz) < depth (&medusa_level) &&
       rn2 (20 + depth (&u.uz)) < 7) 
      mkroom (SHOPBASE);

   else if (depth (&u.uz) > 4 && !rn2 (6))
      mkroom (COURT);
   else if (depth (&u.uz) > 6 && !rn2 (7))
      mkroom (ZOO);
   else if (depth (&u.uz) > 8 && !rn2 (5))
      mkroom (TEMPLE);
   else if (depth (&u.uz) > 9 && !rn2 (4) && // *** GNU was !rn2 (5) ***
            !(mons[PM_KILLER_BEE].geno & (G_GENOD | G_EXTINCT)))
      mkroom (BEEHIVE);
   else if (depth (&u.uz) > 11 && !rn2 (6))
      mkroom (MORGUE);
   else
#ifdef ARMY
      if (depth (&u.uz) > 14 && !rn2 (4) &&
          !(mons[PM_SOLDIER].geno & (G_GENOD | G_EXTINCT)))
      mkroom (BARRACKS);
   else
#endif
   if (depth (&u.uz) > 18 && !rn2 (6))
      mkroom (SWAMP);
...
skip0:
   ...
   for (croom = rooms; croom->hx > 0; croom++)
      ...
      /* Note: monster may be on the stairs. This cannot be avoided:
       * maybe the player fell through a trap door while a monster
       * was on the stairs. Conclusion: we have to check for monsters
       * on the stairs anyway. */
      ...
      /* put box/chest inside; 40% chance for at least 1 box,
       * regardless of number of rooms; about 5 - 7.5% for 2 boxes,
       * least likely when few rooms; chance for 3 or more is
       * neglible. */
      if (!rn2 (nroom * 5 / 2))
...
void mklev ()
   ...
   bound_digging ();
   in_mklev = FALSE;
   /* has_morgue gets cleared once morgue entered; graveyard stays
    * set */
...
register struct mkroom *croom;
   ...
   register int x,
         y,
         roomno = (croom - rooms) + ROOMOFFSET;
   register int lowx = croom->lx,
         lowy = croom->ly;
   register int hix = croom->hx,
         hiy = croom->hy;
   ...
   register int subindex,
         nsubrooms = croom->nsubrooms;

   /* skip the room if already done; i.e. a shop handled out of order */
   /* also skip if this is non-rectangular (it _must_ be done
    * already) */
...
xchar x,
    ...
    * already made one.  This routine can be called twice when a
    * special level is loaded that specifies an SSTAIR location as a
    * favored spot for a branch. */
   if (!br || made_branch)
      return;
   ...
   /* Set made_branch to TRUE even if we didn't make a stairwell
    * (i.e. make_stairs is false) since there is currently only one
    * branch per level, if we failed once, we're going to fail again
    * on the next call. */
   made_branch = TRUE;
   }
...
coord *tm;
   ...
   if (kind == TRAPDOOR && !Can_fall_thru (&u.uz))
      kind = ROCKTRAP;
   ...
   if (kind == WEB)
      (void) makemon (&mons[PM_GIANT_SPIDER], m.x, m.y);
   }
...
void
   ...
   {
   int   dist;
   xchar xmin = inv_pos.x,
         xmax = inv_pos.x;
   xchar ymin = inv_pos.y,
         ymax = inv_pos.y;
   register xchar i;
...
int   dist;
   ...
   unblock_point (x, y);        /* make sure vision knows this
                                 * location is open */
   ...
   /* display new value of position; could have a monster/object on
    * it */
   newsym (x, y);
   }
