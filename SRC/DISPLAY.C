...
#endif                          /* else vobj_at() is defined in
                                 * display.h */
...
int   in_sight;                 /* TRUE if the monster is physically
                                 * seen */
register xchar worm_tail;        /* mon is actually a worm tail */
   ...
   /* We must do the mimic check first.  If the mimic is mimicing
    * something, and the location is in sight, we have to change the
    * hero's memory so that when the position is out of sight, the
    * hero remembers what the mimic was mimicing. */
   ...
   if (mon_mimic && in_sight)
      ...
      switch (mon->m_ap_type)
         ...
         case M_AP_FURNITURE:
               {
               /* This is a poor man's version of map_background().
                * I can't use map_background() because we are
                * overriding what is in the 'typ' field.  Maybe have
                * map_background()'s parameters be (x,y,glyph)
                * instead of just (x,y).
                * 
   ...
   /* If the mimic is unsucessfully mimicing something, display the
    * monster */
   if (!mon_mimic || sensed)
      {
      if (mon->mtame)
...
xchar x,
   ...
   /* If the hero is not in a corridor, then she will feel the wall
    * as a */
   ...
   if (Levitation && !Is_airlevel (&u.uz) && !Is_waterlevel (&u.uz))
      {
      /* Levitation Rules.  It is assumed that the hero can feel the
       * state of the walls around herself and can tell if she is in
       * a corridor, room, or doorway.  Boulders are felt because
       * they are large enough. Anything else is unknown because the
       * hero can't reach the ground. This makes things difficult.
       * 
       * Check (and display) in order:
       * 
       * + Stone, walls, and closed doors. + Boulders.  [see a boulder
       * before a doorway] + Doors. + Room/water positions +
      ...
      if (IS_ROCK (lev->typ) || (IS_DOOR (lev->typ) &&
         ...
         {
         /* An open room or water location.  Normally we wouldn't
          * touch this, but we have to get rid of remembered boulder
          * symbols. This will only occur in rare occations when the
          * hero goes blind and doesn't find a boulder where expected
          * (something came along and picked it up).  We know that
          * there is not a boulder at this location.  Show fountains,
          * pools, etc. underneath if already seen.  Otherwise, show
          * the appropriate floor symbol.
          * 
          * This isn't quite correct.  If the boulder was on top of some
          * other objects they should be seen once the boulder is
          * removed. However, we have no way of knowing that what is
          * there now was there then.  So we let the hero have a
          * lapse of memory. We could also just display what is
          * currently on the top of the object stack (if anything). */
         ...
         map_background (x, y, 1);
         /* Corridors are never felt as lit (unless remembered that
          * way) */
         /* (lit_corridor only).                   */
         if (lev->typ == CORR &&
      ...
      if (Punished)
          ...
          * --- something has been dropped on the ball/chain.  If the
          * bit is not cleared, then when the ball/chain is moved it
          * will drop the wrong glyph. */
      ...
      /* Floor spaces are dark if unlit.  Corridors are dark if
       * unlit. */
      if (lev->typ == ROOM &&
...
register xchar x,
   ...
   if (Underwater && !Is_waterlevel (&u.uz))
      {
      /* don't do anything unless (x,y) is an adjacent underwater
       * position */
      int   dx,
            dy;
   ...
   /* Can't see the location. */
      ...
      /* If the location is remembered as being both dark (waslit is
       * false) and lit (glyph is a lit room or lit corridor) then it
       * was either:
       * 
       ...
       * 
       * In either case, we have to manually correct the hero's memory
       * to match waslit.  Deciding when to change waslit is
       * non-trivial.
       * 
       * Note:  If flags.lit_corridor is set, then corridors act like
       * room squares.  That is, they light up if in night vision
       * range. If flags.lit_corridor is not set, then corridors will
       * remain dark unless lit by a light spell.
       * 
       * These checks and changes must be here and not in
       * back_to_glyph(). They are dependent on the position being
       * out of sight. */
      else if (!lev->waslit)
         {
...
xchar x,
   ...
   if (cansee (x, y))
      {                             /* Don't see anything if can't see
                                 * the location */
   ...
   switch (x)
      ...
      case DISP_END:
         if (status == DISP_BEAM)
            ...
            {                       /* DISP_FLASH (called at least once) */
            newsym (sx, sy);     /* reset the location */
            sx = -1;              /* reset sx to an illegal pos for
                                 * next time */
            }
         break;
...
void
      curs_on_u ()
   {
   flush_screen (1);              /* Flush waiting glyphs & put cursor
                                 * on hero */
   }
...
void
    ...
    * physical screen with the symbol for rock + clears the glyph
    * buffer */
   cls ();
...
typedef struct
   {
   xchar new;                    /* perhaps move this bit into the rm
                                 * strucure. */
   int   glyph;
   }     gbuf_entry;
...
int   glyph;
   ...
   if (x <= 0 || x >= COLNO || y < 0 || y >= ROWNO)
      ...
      /* column 0 is invalid, but it's often used as a flag, so
       * ignore it */
      if (x == 0)
         return;
...
void
   ...
   clear_glyph_buffer ();        /* this is sort of an extra effort,
                                 * but OK */
   }
...
xchar x,
   ...
   switch (ptr->typ)
      ...
      case CORR:
         ...
         break;
      case HWALL:
         idx = ptr->seen ? S_hwall : S_stone;
         break;
      case VWALL:
         idx = ptr->seen ? S_vwall : S_stone;
         break;
      case TLCORNER:
         idx = ptr->seen ? S_tlcorn : S_stone;
         break;
      case TRCORNER:
         idx = ptr->seen ? S_trcorn : S_stone;
         break;
      case BLCORNER:
         idx = ptr->seen ? S_blcorn : S_stone;
         break;
      case BRCORNER:
         idx = ptr->seen ? S_brcorn : S_stone;
         break;
      case CROSSWALL:
         idx = ptr->seen ? S_crwall : S_stone;
         break;
      case TUWALL:
         idx = ptr->seen ? S_tuwall : S_stone;
         break;
      case TDWALL:
         idx = ptr->seen ? S_tdwall : S_stone;
         break;
      case TLWALL:
         idx = ptr->seen ? S_tlwall : S_stone;
         break;
      case TRWALL:
         idx = ptr->seen ? S_trwall : S_stone;
         break;
      case SDOOR:
         if (ptr->seen)
