...
int   min_rx,
      max_rx,
      min_ry,
      max_ry;                    /* rectangle bounds for regions */
...
joinm:
   /* Ok, now we can actually join the regions with fg_typ's. The
    * rooms are already sorted due to the previous loop, so don't
    * call sort_rooms(), which can screw up the roomno's validity in
    * the levl structure. */
   for (croom = &rooms[0], croom2 = croom + 1; croom2 < &rooms[nroom];)
      ...
      if (!somexy (croom, &sm) || !somexy (croom2, &em))
         ...
         /* arbitrarily pick centers of both rooms and hope for the
          * best */
