...
pick_move:
   ...
   if (mtmp->ispriest && avoid &&
      ...
      {
      /* might as well move closer as long it's going to stay lined
       * up */
      avoid = FALSE;
      goto pick_move;
      }
   ...
   if (!temple_occupied (u.urooms0))
      ...
      if (tended)
         ...
         if (sanctum && Is_sanctum (&u.uz))
            ...
            verbalize ("Pilgrim, you enter a%s place!",
                       !shrined ? " desecrated" :
                       " sacred");
         if (!sanctum)
            {
...
register struct monst *priest;
   ...
   if (!u.ugold)
      ...
      if ((offer = bribe (priest)) == 0)
         ...
         if (u.ugold < (offer * 2L))
            {
            if (coaligned && u.ualign.record < -5)
               u.ualign.record++;
...
void
      ghod_hitsu (priest)        /* when attacking "priest" in his
                                 * temple */
struct monst *priest;
   {
   int   x,
         ...
         ax,
         ay,
         roomno = (int) temple_occupied (u.urooms);
   struct mkroom *troom;
