...
# define COPY_OBJ_DESCR(o_dst,o_src) o_dst.oc_descr_idx = o_src.oc_descr_idx
   ...
   /* moved to after u_init() init_artifacts(); */
...
check:
      ...
      if (OBJ_DESCR (objects[first]) != NULL &&
         ...
         if (acls == GEM_CLASS)
            {
            if (rn2 (2))
               {                    /* change turquoise from green to
                                 * blue? */
               COPY_OBJ_DESCR (objects[TURQUOISE], objects[SAPPHIRE]);
               }
            if (rn2 (2))
               {                    /* change aquamarine from green to
                                 * blue? */
            ...
            /* non-magical amulets, scrolls, and spellbooks (ex.
             * imitation Amulets, blank, scrolls of mail) and
             * one-of-a-kind magical artifacts at the end of their
             * class in objects[] have fixed descriptions. */
...
int
   ...
   for (i = SPEED_BOOTS; i <= LEVITATION_BOOTS; i++)
      if ((s = OBJ_DESCR (objects[i])) != 0 && !strcmp (s, "snow boots"))
         return i;
   /* special case:  description of elven boots is nulled out for elf
    * role */
   if (pl_character[0] == 'E')
      return ELVEN_BOOTS;

   impossible ("snow boots not found?");
   return -1;                    /* not 0, or caller would try again
                                 * each move */
   }
...
register int fd;
   ...
   /* as long as we use only one version of Hack we need not save
...
boolean mark_as_known;
   ...
   if (!objects[oindx].oc_name_known)
      {
      register int dindx,
            acls = objects[oindx].oc_class;

      /* Loop thru disco[] 'til we find the target (which may have
       * been uname'd) or the next open slot; one or the other will
       * be found before we reach the next class... */
...
int
   ...
   /* several classes are omitted from packorder; one is of interest
    * here */
