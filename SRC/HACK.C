      ...
      if (isok (rx, ry) && !IS_ROCK (levl[rx][ry].typ) &&
         ...
         if (ttmp)
            switch (ttmp->ttyp)
               ...
               case PIT:
                  freeobj (otmp);
                  /* vision kludge to get messages right; the pit
                   * will temporarily be seen even if this is one
                   * among multiple boulders */
                  if (!Blind)
         ...
         if (boulder_hits_pool (otmp, rx, ry, TRUE))
            continue;
         /* Re-link at top of fobj chain so that pile order is
...
#endif
            /* note: this var contains garbage initially and after a
             * restore */
...
xchar x,
   ...
   register const char *digtxt = (const char *) 0,
        *dmgtxt = (const char *) 0;
   ...
   if (boulder)
      ...
      /* The location could still block because of 1. More than one
       * boulder 2. Boulder stuck in a wall/stone/door.
       * 
...
void
   ...
   xchar chainx,
         chainy,
         ballx,
         bally;                 /* ball&chain new positions */
   ...
   if (u.uswallow)
      ...
      if (u.ustuck && (x != u.ustuck->mx ||
         ...
         if (distu (u.ustuck->mx, u.ustuck->my) > 2)
            ...
            {
#ifdef POLYSELF
            /* If polymorphed into a sticking monster, u.ustuck means
   ...
   if (mtmp)
      ...
      /* or if it hides_under, in which case we call attack() to
       * print the Wait! message. This is different from ceiling
       * hiders, who aren't handled in attack(). */
    ...
    * there are special extenuating circumstances: (1) if the pet
    * dies then your god angers, (2) if the pet gets trapped then
    * your god may disapprove, (3) if the pet was already trapped and
    * you attempt to free it not only do you encounter the trap but
    * you may frighten your pet causing it to go wild!  moral: don't
    * abuse this privilege. */
   /* Ceiling-hiding pets are skipped by this section of code, to be
    * caught by the normal falling-monster code. */
   if (is_safepet (mtmp) && !(is_hider (mtmp->data) && mtmp->mundetected))
      ...
      /* check first to see if monster drowned. then check for traps. */
      if (minwater (mtmp))
         {
         swap_result = 2;
         }
      else
         swap_result = mintrap (mtmp);
      ...
      switch (swap_result)
         ...
         case 3:                 /* changed levels */
            ...
            break;
         case 2:
            /* it may have drowned or died.  that's no way to treat a
             * pet!  your god gets angry and complains. */
            if (rn2 (4))
               {
   ...
   if (u.ux0 != u.ux || u.uy0 != u.uy)
      {
      u.umoved = TRUE;
      /* Clean old position -- vision_recalc() will print our new
       * one. */
      newsym (u.ux0, u.uy0);
      /* Since the hero has moved, adjust what can be seen/unseen. */
      vision_recalc (1);        /* Do the work now in the recover
                                 * time. */
...
register boolean newlev;
   ...
   for (ptr = &u.uentered[0]; *ptr; ptr++)
      {
      register int roomno = *ptr - ROOMOFFSET,
            rt = rooms[roomno].rtype;

      /* Did we just enter some other special room? */
      /* vault.c insists that a vault remain a VAULT, and temples
       * should remain TEMPLEs, but everything else gives a message
       * only the first time */
      switch (rt)
         {
         case ZOO:
...
void
   ...
   register int x,
         ...
         m0 = 1,
         i0 = 9;
   register int corrct = 0,
         noturn = 0;
   register struct monst *mtmp;
   register struct trap *trap;

#ifdef POLYSELF
   /* Grid bugs stop if trying to move diagonal, even if blind.
    * Maybe */
   ...
   for (x = u.ux - 1; x <= u.ux + 1; x++)
      ...
      bcorr:
            ...
            {
            /* water and lava only stop you if directly in front, and
             * stop you even if you are running */
            if (!Levitation &&
#ifdef POLYSELF
                !is_flyer (uasmon) && !is_clinger (uasmon) &&
#endif
