...
void
   ...
   /* Rogue levels are structured 3 by 3, with each section
    * containing a room or an intersection.  The minimum width is 2
    * each way. One difference between these and "real" Rogue levels:
    * real Rogue uses 24 rows and NetHack only 23.  So we cheat a bit
    * by making the second row of rooms not as deep.
    * 
    ...
    * Not counting walls, the room may go in rows 1-5 and columns
    * 1-23 (numbering starting at 0).  A room coordinate of this type
    * may be converted to a level coordinate by adding 1+28*x to the
    * column, and 7*y to the row.  (The 1 is because column 0 isn't
    * used [we only use 1-78]). Room height may be 2-4 (2-5 on last
    * row), length 2-23 (not counting walls) */
#define here r[x][y]
