...
struct obj *box;
   ...
   for (n = rn2 (n + 1); n > 0; n--)
      ...
      if (box->otyp == ICE_BOX)
         ...
         for (tprob = rnd (100); (tprob -= iprobs->iprob) > 0; iprobs++)
            ;
         if (!(otmp = mkobj (iprobs->ilet, TRUE)))
            continue;
   ...
   if (gold)
      gold->owt = weight (gold);/* quantity was diddled */
   return;
   }

int
      rndmonnum ()              /* select a random, common monster
                                 * type */
   {
...
register struct obj *obj;
   ...
   if (Is_container (obj) || obj->otyp == STATUE)
      ...
      for (contents = obj->cobj; contents; contents = contents->nobj)
         cwt += weight (contents);
      /* The weight of bags of holding is calculated as the weight of
       * the bag plus the weight of the bag's contents modified as
       * follows:
       * 
       * Bag status   Weight of contents ----------   ------------------
       * cursed         2x blessed         x/4 + 1 otherwise      x/2 +
       * 1
       * 
       * The macro DELTA_CWT in pickup.c also implements these weight
       * equations.
       * 
       * Note:  The above checks are performed in the given order. this
       * means that if an object is both blessed and cursed (not
       * supposed to happen), it will be treated as cursed. */
