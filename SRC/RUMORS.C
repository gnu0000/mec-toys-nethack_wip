...
static long true_rumor_start,
      ...
      false_rumor_size,
      false_rumor_end;
static int oracle_flg = 0;        /* -1=>don't use, 0=>need init,
                                 * 1=>init done */
...
FILE *fp;
   ...
   (void) fgets (line, sizeof line, fp);   /* skip "don't edit"
                                           * comment */
   if (fscanf (fp, "%6lx\n", &true_rumor_size) == 1 &&
       true_rumor_size > 0L)
      {
...
int   truth;                    /* 1=true, -1=false, 0=either */
   ...
   if (rumors)
      ...
      /* input:      1    0   -1 rn2 \ +1  2=T  1=T  0=F adj./ +0
       * 1=T  0=F -1=F */
      switch (truth += rn2 (2))
         {
         case 2:                 /* (might let a bogus input arg sneak
                                 * thru) */
         case 1:
            beginning = true_rumor_start;
            tidbit = Rand () % true_rumor_size;
            break;
         case 0:                 /* once here, 0 => false rather than
                                 * "either" */
         case -1:
...
boolean special;
   ...
   if (oracle_flg < 0 ||        /* couldn't open ORACLEFILE */
       (oracle_flg > 0 && oracle_cnt == 0))   /* oracles already
                                              * exhausted */
      return;
...
register struct monst *oracl;
   ...
   flags.botl = 1;
   add_xpts = 0;                 /* first oracle of each type gives
                                 * experience points */
   if (u_pay == minor_cost)
      {
      outrumor (1, FALSE);
