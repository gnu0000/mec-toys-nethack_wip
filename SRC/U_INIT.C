...
const char *roles[] =    {           /* also used in options.c and
                                 * winxxx.c */
...
void
   ...
   /* zero u, including pointer values -- necessary when aborting
    * from a failed restore */
   (void) memset ((genericptr_t) & u, 0, sizeof (u));
   u.ustuck = (struct monst *) 0;
   u.usick_cause = NULL;

#if 0                             /* documentation of more zero values
                                 * as desirable */
...
# endif
   ...
   u.ulevel = 0;                 /* set up some of the initial
                                 * attributes */
   ...
   switch (pc)
      ...
      case 'A':
         ...
         if (!rn2 (10))
            ini_inv (Tinopener);
         else if (!rn2 (3)) // GNU was rn2 (4)
            ini_inv (Lamp);
         else if (!rn2 (5)) // GNU was rn2 (10)
            ini_inv (Magicmarker);
      ...
      case 'E':
          ...
          * generators are bad enough to seriously skew the results
          * if we use rn2(2)...  --KAA */
         ini_inv (Elf);
...
register struct trobj *trop;
   ...
   while (trop->trclass)
       ...
       * i.e. the discovery wishing, are still OK. Also, don't get a
       * couple of really useless items.  (Note: punishment isn't
       * "useless".  Some players who start out with one will
       * immediately read it and use the iron ball as a weapon.) */
      if (undefined)
         {
#ifdef POLYSELF
         ...
         /* Heavily relies on the fact that 1) we create wands before
          * rings, 2) that we create rings before spellbooks, and
          * that 3) not more than 1 object of a particular symbol is
          * to be prohibited.  (For more objects, we need more
          * nocreate variables...) */
#ifdef POLYSELF
         switch (obj->otyp)
            {
      ...
      /* The below lines not needed because they don't correspond to
       * any actual inventory; nobody gets random tools. else
       * if(obj->oclass == TOOL_CLASS && undefined)    { obj->bknown =
       * (obj->otyp != BAG_OF_TRICKS && obj->otyp != SACK &&
       * obj->otyp != CHEST && obj->otyp != LARGE_BOX && obj->otyp !=
       * ICE_BOX);    } */
      ...
      /* below changed by GAN 01/09/87 to allow wielding of pick-axe
       * or can-opener if there is no weapon */
...
void
   ...
   if (pl_character[0] == '@')
      {                             /* explicit request for random class */
