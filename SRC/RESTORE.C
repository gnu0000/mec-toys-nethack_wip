...
static void
   ...
   /* Reverse the entire fobj chain, which is necessary so that we
    * can place the objects in the proper order. */
   while ((otmp = fobj) != 0)
      {
      fobj = otmp->nobj;
      otmp->nobj = fobjtmp;
 ...
 */
   ...
   for (otmp = invent; otmp; otmp = otmp2)
      ...
      if (otmp->in_use)
         {
         /* in_use and oldcorpse share a bit, but we don't want nasty
          * messages for old corpses -- remove_cadavers() will clean
          * them up nicely */
         if (otmp->otyp == CORPSE &&
...
boolean ghostly;
   ...
   while (1)
      ...
      if (u.uz.dlevel)
          ...
          * shop_keeper().  just wait and process the damage on the
          * second pass. */
...
unsigned int *mid;
   ...
   if (u.uhp <= 0)
      ...
      /* wiz1_level.dlevel is used by mklev.c to see if lots of stuff
       * is uninitialized, so we only have to set it and not the
       * other stuff. */
      wiz1_level.dlevel = 0;
      u.uz.dnum = 0;
      u.uz.dlevel = 1;
...
# pragma unused(fd)
   ...
   if (nfd < 0)
      panic ("Cannot open temp level %d!", ltmp);
#ifdef MFLOPPY
   if (!savelev (nfd, ltmp, COUNT_SAVE))
      {

      /* The savelev can't proceed because the size required is
       * greater than the available disk space. */
...
# ifndef AMIGA
      ...
      /* Perhaps the person would like to play without a RAMdisk. */
      if (ramdisk)
         {
         /* PlaywoRAMdisk may not return, but if it does it is
          * certain that ramdisk will be 0. */
         playwoRAMdisk ();
...
# ifdef AMII_GRAPHICS
      ...
      if (windowprocs.win_init_nhwindows == amii_procs.win_init_nhwindows)
         ...
         clear_nhwindow (WIN_BASE);   /* hack until there's a hook for
                                     * this */
         }
      }
# else
...
monfnd:
   ...
   for (otmp = fobj; otmp; otmp = otmp->nobj)
      if (otmp->owornmask)
         setworn (otmp, otmp->owornmask);
#ifndef NO_SIGNAL
   /* in_use processing must be after: + The inventory has been read
    * so that freeinv() works. + The current level has been restored
    * so billing information is available. */
   inven_inuse ();
#endif
#ifdef MULDGN
...
/* CHECK:  This may prevent restoration */
...
#if defined(MAC)
      memset (&r, 0, sizeof (r));   /* Suppress warning about used
                                     * before set */
#endif
      i = 0;
      j = 0;
   ...
   /* regenerate animals while on another level */
      ...
      for (mtmp = fmon; mtmp; mtmp = mtmp2)
         ...
         if (mtmp->data->geno & G_GENOD)
            {
            /* mondead() would try to link the monster's objects into
             * fobj and the appropriate nexthere chain.
             * unfortunately, such things will not have sane values
             * until after find_lev_obj() well below here, so we'd go
             * chasing random pointers if we tried that.  we could
             * save the monster's objects in another chain and insert
             * them in the level later, but that's a lot of work for
             * very little gain.  hence, just throw the objects away
             * via mongone() and pretend the monster wandered off
   ...
   if (ghostly && (br = Is_branchlev (&u.uz)) && u.uz.dlevel == 1)
      ...
      switch (br->type)
         ...
         case BR_STAIR:
         case BR_NO_END1:
         case BR_NO_END2:        /* OK to assign to sstairs if it's
                                 * not used */
...
#ifdef ZEROCOMP
#define RLESC '\0'              /* Leading character for run of
                                 * RLESC's */
