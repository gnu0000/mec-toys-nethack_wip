...
#include <errno.h>
#ifdef _MSC_VER                 /* MSC 6.0 defines errno quite
                                 * differently */
# if (_MSC_VER >= 600)
#  define SKIP_ERRNO
# endif
...
void
      doaltarobj (obj)           /* obj is an object dropped on an
                                 * altar */
...
STATIC_OVL
void
      dosinkring (obj)           /* obj is a ring being dropped over a
                                 * kitchen sink */
register struct obj *obj;
   {
...
#ifndef NO_SIGNAL
   obj->in_use = TRUE;           /* block free identification via
                                 * interrupt */
#endif
   switch (obj->otyp)
...
register struct obj *obj;
   ...
   if (obj == uwep)
      ...
      if (uwep)
         return 0;              /* lifesaved and rewielded */
      }
#ifdef SINKS
...
register struct obj *obj;
   ...
   if (!u.uswallow && obj != uball &&
       ship_object (obj, u.ux, u.uy, FALSE))
      return;
   dropy (obj);
   }
...
static int
   ...
   /* since level change might be a bit slow, flush any buffered
    * screen output (like "you fall through a trapdoor") */
   mark_synch ();
   ...
   if (fd < 0)
      {
      /* This is not quite impossible: e.g., we may have exceeded our
       * quota. If that is the case then we cannot leave this level,
       * and cannot save either. Another possibility is that the
...
register boolean at_stairs,
   ...
   /* In Nethack 3.1, Gehennom starts after the stronghold.
    * Moreover, there are traps in the stronghold, that can send the
    * player to Gehennom (gnark, gnark)!  So we have to test here: */
      ...
      if (Conflict)
         ...
         if (enexto (&mm, mm.x, mm.y, &mons[PM_ANGEL]))
            ...
            if ((mtmp = mk_roamer (&mons[PM_ANGEL], u.ualign.type,
               ...
               /* guardian angel -- the one case mtame doesn't imply
                * an edog structure, so we don't want to call
                * tamedog(). */
               mtmp->mtame = 10;
...
void
   ...
   if (!on_level (&u.uz, &u.utolev))
      {
      d_level dest;
      int   typmask = u.utotype;/* save it; goto_level zeroes
                                 * u.utotype */
 ...
 */
#define TAINT_AGE   50              /* min. limit for tainting.  see
                                 * eat.c */
#define ODDS_RV1   37              /* 1/37 odds for 50 moves = 75%
                                 * revive */
   ...
   for (obj = *chain; obj; obj = nobj)
      ...
      if (obj->otyp == CORPSE)
         ...
         if (is_rider (&mons[obj->corpsenm]) && corpse_age >= 12 && onfloor)
            ...
            /* riders can't be picked up, but can be buried, but the
             * astral level seems to rule out that possibility */
             ...
             * level and return to old corpses that haven't been
             * marked they're given a one-shot chance to re-animate. */
            if (corpse_age < TAINT_AGE)
               revive_corpse (obj, ODDS_RV1, FALSE);
            else if (corpse_age == TAINT_AGE)
               revive_corpse (obj, ODDS_RV1, TRUE);
            else
               revive_corpse (obj, ODDS_RV2, TRUE);
      ...
      /* pobj is only used for containers, which don't allow revive()
       * -dlc */
...
int
      donull ()
   {
   return (1);                    /* Do nothing, but let other things
                                 * happen */
   }
...
int
   ...
   if (u.ucreamed)
      ...
      /* Not totally correct; what if they change back after now but
       * before they're finished wiping? */
      return (1);
      }
