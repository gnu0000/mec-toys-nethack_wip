...
boolean restore;
   ...
   for (otmp = ochain; otmp; otmp = otmp->nobj)
      ...
      if (!restore)
         {
         /* resetting the o_id's after getlev has carefully created
          * proper new ones via restobjchn is a Bad Idea */
         otmp->o_id = 0;
         ...
         if (otmp->otyp == AMULET_OF_YENDOR)
            ...
            {
            /* never leave our own quest artifact among the bones;
             * others (via wishing) might remain though */
...
void
   ...
   if (no_bones_level (&u.uz))
      return;                    /* no bones for specific levels */
   ...
   if (depth (&u.uz) <= 0 ||     /* bulletproofing for endgame */
       (!rn2 (1 + (depth (&u.uz) >> 2))   /* fewer ghosts on low
                                           * levels */
#ifdef WIZARD
        && !wizard
#endif
   ...
   if (u.ugrave_arise == -2)
      ...
      /* trick makemon() into allowing monster creation on your
       * location */
      in_mklev = TRUE;
...
# ifdef TUTTI_FRUTTI
    ...
    * initializes bytes_counted to 0, so doing savefruitchn() first
    * is useless; the extra bflush() at the end of savelev() may
    * increase bytes_counted by a couple over what the real usage
    * will be */
   savefruitchn (fd, COUNT_SAVE);
   bflush (fd);
# endif
   if (bytes_counted > freediskspace (bones))
      {                             /* not enough room */
# ifdef WIZARD
      if (wizard)
...
# endif
   ...
   co_false ();                 /* make sure bonesid and savefruitchn
                                 * get written */
#endif                          /* MFLOPPY */
...
int
   ...
   if (rn2 (3)                    /* only once in three times do we
                                 * find bones */
#ifdef WIZARD
       && !wizard
#endif
