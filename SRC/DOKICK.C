...
register xchar x,
   ...
   setmangry (mon);
#ifdef POLYSELF
   /* Kick attacks by kicking monsters are normal attacks, not
    * special. If you have >1 kick attack, you get all of them. */
   ...
   /* no need to check POLYSELF since only ghosts, which you can't
    * turn */
...
xchar x,
   ...
   if (Is_box (kickobj))
      ...
      if (kickobj->olocked)
         ...
         if (!rn2 (5) || (martial () && !rn2 (2)))
            ...
            kickobj->obroken = 1;
            if (otrp)
               (void) chest_trap (kickobj, LEG, FALSE);
            return (1);
            }
         }
      else
   ...
   /* too heavy to move.  range is calculated as potential distance
    * from player, so range == 2 means the object may move up to one
    * square from its current position */
   ...
   if (costly &&
      ...
      if (isgold)
         ...
         (void) stolen_value (kickobj, x, y, FALSE, FALSE);
      }
...
static char *
   ...
   if (kickobj)
      Sprintf (buf, "kicking %s", doname (kickobj));
   else
      {
      Strcpy (buf, "kicking ");
      ...
      if (IS_THRONE (maploc->typ))
         ...
         if ((Luck < 0 || maploc->doormask) && !rn2 (3))
            ...
            if (i > 6)
               i = 6;
            while (i--)
               (void) mkobj_at (GEM_CLASS, x, y, TRUE);
            if (Blind)
...
register xchar gate;
   ...
   switch (gate)
      ...
      case 0:
      case 4:
         optr = "through the trap door.";
         break;
      case 1:
      case 3:
         optr = "down the stairs.";
         break;
      case 2:
         optr = "down the ladder.";
         break;
      default:
         optr = "down out of sight.";
         break;
      }
   return (optr);
   }
...
register xchar loc;
   ...
   switch (loc)
      {
      case 0:
         if (In_endgame (&u.uz) || (Is_botlevel (&u.uz) &&
                                    !Is_stronghold (&u.uz)))
            {
            cc->y = 0;
            return;
...
register xchar x,
   ...
   angry = FALSE;
   shkp = (struct monst *) 0;
   /* if 'costly', we must keep a record of ESHK(shkp) before it
    * undergoes changes through the calls to stolen_value. the angry
    * bit must be reset, if needed, in this fn, since stolen_value is
    * called under the 'silent' flag to avoid unsavory pline
    * repetitions. */
   if (costly)
      {
...
chain_reaction:
   if (impact)
      {
      /* the objs impacted may be in a shop other than the one in
       * which the hero is located.  another check for a shk is made
       * in impact_drop.  it is, e.g., possible to kick/throw an
       * object belonging to one shop into another shop through a gap
       * in the wall, and cause objects belonging to the other shop
       * to fall down a trapdoor--thereby getting two shopkeepers
   ...
   for (otmp = migrating_objs; otmp; otmp = otmp2)
      ...
      if (otmp->ox == u.uz.dnum && otmp->oy == u.uz.dlevel)
         ...
         common:
            ...
            default:
         scatter:
               /* set dummy coordinates because there's no current
                * position for rloco() to update */
               otmp->ox = otmp->oy = 0;
               rloco (otmp);
               break;
            }
...
#ifdef MULDGN                    /* this matches the player
                                 * restriction in goto_level() */
   if (on_level (&u.uz, &qstart_level) && !ok_to_quest ())
      return -1;
#endif
   if (ttmp && ttmp->ttyp == TRAPDOOR && ttmp->tseen)
      return 0;
   if (xdnstair == x && ydnstair == y)
      return 1;
   if (xdnladder == x && ydnladder == y)
      return 2;
   if (sstairs.sx == x && sstairs.sy == y && !sstairs.up)
      return 3;
   return -1;
   }
