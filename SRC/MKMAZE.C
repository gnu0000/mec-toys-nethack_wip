...
static void
      maze0xy (cc)              /* find random starting point for
                                 * maze generation */
coord *cc;
   {
...
d_level *lev;
   ...
   if (!lx)
      ...
      /* if there are rooms and this a branch, let place_branch
       * choose the branch location (to avoid putting branches in
       * corridors). */
...
static void
   ...
   if (was_waterlevel)
      ...
      was_waterlevel = TRUE;
      /* water level is an odd beast - it has to be set up before
       * calling place_lregions etc. */
      setup_waterlevel ();
      }
...
void
      mazexy (cc)                 /* find random point in generated
                                 * corridors, so we don't create
                                 * items in moats, bunkers, or walls */
coord *cc;
   {
   int   cpt = 0;
 ...
 */
   ...
   /* Don't bound these until _after_ the previous loops to avoid
    * "ice" */
...
static struct bubble
   ...
   xchar x,
         y;                       /* coordinates of the upper left
                                 * corner */
   schar dx,
         dy;                    /* the general direction of the
                                 * bubble's movement */
   uchar *bm;                    /* pointer to the bubble bit mask */
   struct bubble *prev,
        *next;                    /* need to traverse the list up and
                                 * down */
...
void
   ...
   /* Every second time traverse down.  This is because otherwise all
    * the junk that changes owners when bubbles overlap would
    * eventually end up in the last bubble in the chain. */

   up = !up;
   for (b = up ? bbubbles : ebubbles; b; b = up ? b->next : b->prev)
      {
      register int rx = rn2 (3),
            ry = rn2 (3);
...
register int fd;
   {
   register struct bubble *b = (struct bubble *) 0,
        *btmp;
   register int i;
   int   n;
...
register int x,
      ...
      n;
   {
   /* These bit masks make visually pleasing bubbles on a normal
    * aspect 25x80 terminal, which naturally results in them being
    * mathematically anything but symmetric.  For this reason they
    * cannot be computed in situ, either.  The first two elements
    * tell the dimensions of the bubble's bounding box. */
   static uchar
         bm2[] =    {2, 1, 0x3   },
         bm3[] =    {3, 2, 0x7, 0x7   },
         bm4[] =    {4, 3, 0x6, 0xf, 0x6   },
...
register boolean ini;
   ...
   /* collision with level borders? 1 = horizontal border, 2 =
    * vertical, 3 = corner */
   if (b->x <= bxmin)
      colli |= 2;
   if (b->y <= bymin)
   ...
   switch (colli)
      ...
      case 2:
         ...
         break;
      default:
         /* sometimes alter direction for fun anyway (higher
          * probability for stationary bubbles) */
