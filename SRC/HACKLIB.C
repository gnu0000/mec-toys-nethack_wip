...
#ifdef OVLB
char *
      lcase (s)                 /* convert a string into all
                                 * lowercase */
char *s;
   {
   register char *p;
...
#ifdef OVL0
char *
      eos (s)                    /* return the end of a string
                                 * (pointing at '\0') */
register char *s;
   {
   while (*s)
...
char *
      s_suffix (s)              /* return a name converted to
                                 * possessive */
const char *s;
   {
...
char *
      xcrypt (str)              /* trivial text encryption routine
                                 * (see makedefs) */
const char *str;
   {
   Static char buf[BUFSZ];
...
#ifdef OVLB
char *
      tabexpand (sbuf)           /* expand tabs into proper number of
                                 * spaces */
char *sbuf;
   {
   char  buf[BUFSZ];
...
char *
      visctrl (c)                 /* make a displayable string from a
                                 * character */
char  c;
   {
   Static char ccc[3];
...
#ifdef OVL2
const char *
      ordin (n)                 /* return the ordinal suffix of a
                                 * number */
...
#ifdef OVL1
char *
      sitoa (n)                 /* make a signed digit string from a
                                 * number */
int   n;
   {
   Static char buf[13];
...
int
      sgn (n)                    /* return the sign of a number: -1,
                                 * 0, or 1 */
int   n;
   {
   return (n < 0) ? -1 : (n != 0);
...
#ifdef OVLB
int
      rounddiv (x, y)           /* calculate x/y, rounding as
                                 * appropriate */
long  x;
int   y;
   {
...
#ifdef OVL0
int
      distmin (x0, y0, x1, y1)  /* distance between two points, in
                                 * moves */
int   x0,
      y0,
      x1,
   ...
   if (dy < 0)
      dy = -dy;
   /* The minimum number of moves to get from (x0,y0) to (x1,y1) is
    * the :  larger of the [absolute value of the] two deltas. */
   return (dx < dy) ? dy : dx;
   }

int
      dist2 (x0, y0, x1, y1)     /* square of euclidean distance
                                 * between pair of pts */
int   x0,
      y0,
      x1,
...
boolean
online2 (x0, y0, x1, y1)        /* are two points lined up (on a
                                 * straight line)? */
int   x0,
      y0,
      x1,
    ...
    * else if the deltas are equal (signs ignored) they're on a
    * diagonal. */
...
const char *patrn,
    ...
    * '?' matches :  any single character.  Returns TRUE if 'strng'
    * matches 'patrn'. */
pmatch_top:
   s = *strng++;
   p = *patrn++;                 /* get next chars and pre-advance */
   if (!p)                       /* end of pattern */
      return ((boolean) (s == '\0'));   /* matches iff end of string
                                        * too */
...
#ifndef STRNCMPI
int
      strncmpi (s1, s2, n)        /* case insensitive counted string
                                 * comparison */
register const char *s1,
     *s2;
register int n;                 /* (should probably be size_t, which
                                 * is usually unsigned) */
   {                                /*    { aka strncasecmp    } */
   register char t1,
         t2;
...
# define TABSIZ 0x20              /* 0x40 would be case-sensitive */
   char  tstr[TABSIZ],
         tsub[TABSIZ];           /* nibble count tables */

# if 0
   assert ((TABSIZ & ~(TABSIZ - 1)) == TABSIZ);   /* must be exact power
                                                 * of 2 */
...
# endif
   ...
   if (k < 0)
      return (char *) 0;        /* sub longer than str, so can't
                                 * match */
   for (i = 0; i < TABSIZ; i++)  /* does sub have more 'x's than str? */
      if (tsub[i] > tstr[i])
         return (char *) 0;     /* match not possible */

   /* now actually compare the substring repeatedly to parts of the
    * string */
   for (i = 0; i <= k; i++)
      {
      s1 = &str[i];
