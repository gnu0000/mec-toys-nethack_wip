...
STATIC_VAR NEARDATA long int followmsg;   /* last time of follow
                                           * message */
...
register struct monst *mtmp;
   ...
   /* make sure bill is set only when the dead shk is the resident
    * shk.    */
...
STATIC_OVL void
      setpaid (shkp)              /* either you paid or left the shop
                                 * or the shopkeeper died */
register struct monst *shkp;
   {
...
register boolean newlev;
   ...
   /* IF player ((didn't leave outright) AND ((he is now
    * strictly-inside the shop) OR (he wasn't strictly-inside last
    * turn anyway))) THEN (there's nothing to do, so just return) */
   if (!*leavestring &&
   ...
   if (!*leavestring && shkp->mcanmove && !shkp->msleep)
      {
      /* Player just stepped onto shop-boundary (known from above
       * logic). Try to intimidate him into paying his bill */
...
struct obj *obj1,
     *obj2;
   {
   register struct monst *shkp1,
        *shkp2;
   register struct bill_x *bp1 = 0,
        *bp2 = 0;
   ...
   for (shkp1 = next_shkp (fmon, TRUE); shkp1;
      ...
      if ((bp1 = onbill (obj1, shkp1, TRUE)) != 0)
         break;
   /* second object is probably owned by same shk; if not, look
    * harder */
...
register boolean killkops;
   {
   register xchar x = ESHK (shkp)->shk.x,
         y = ESHK (shkp)->shk.y;
...
register xchar ox,
   ...
   if (!ANGRY (shkp))
      ...
      pline ("%s is furious!", Monnam (shkp));
   hot_pursuit (shkp);
   }
...
static long
      cheapest_item (shkp)        /* delivers the cheapest item on the
                                 * list */
register struct monst *shkp;
   {
   register int ct = ESHK (shkp)->billct;
...
int
   ...
   register struct monst *shkp,
        *resident = (struct monst *) 0;
   register struct eshk *eshkp;
   int   pass,
         tmp,
         sk = 0,
         seensk = 0;
   register boolean paid = FALSE,
         stashed_gold = (hidden_gold () > 0L);
...
proceed:
   ...
   if (!eshkp->billct && !eshkp->debit)
      ...
      if (!ltmp && NOTANGRY (shkp))
         ...
         {
         /* shopkeeper is angry, but has not been robbed -- door
          * broken, attacked, etc. */
      ...
      for (pass = 0; pass <= 1; pass++)
         ...
         while (tmp < eshkp->billct)
            ...
            if (pass == bp->useup && otmp->quan == bp->bquan)
                ...
                * second, so player will be stuck in the store less
                * often; things which are partly used up are
                * processed on both passes */
               tmp++;
               }
            else
               {
...
register struct bill_x *bp;
struct obj **obj_p;
int   which;                    /* 0 => used-up item, 1 => other
                                 * (unpaid or lost) */
boolean itemize;
   {
   ...
   if (consumed)
      ...
      if (quan != bp->bquan)
         ...
         {                          /* completely used-up, so get rid of
                                 * it */
         if (obj == billobjs)
            {
            billobjs = obj->nobj;
            ...
            if (otmp)
               ...
               impossible ("Error in shopkeeper administration.");
            }
         /* assert( obj == *obj_p ); */
         dealloc_obj (obj);
   ...
   if (eshkp->following || ANGRY (shkp) || take)
      ...
      if ((loss > u.ugold) || !loss)
         ...
         if (!*u.ushops ||
            ...
            {
            /* shk.x,shk.y is the position immediately in front of
             * the door -- move in one more space */
            ox = eshkp->shk.x;
            oy = eshkp->shk.y;
         ...
         if (invent)
            ...
            /* add to main object list at end so invent is still good */
            if (fobj)
               {
               otmp = fobj;
               while (otmp->nobj)
   ...
   if (!objects[obj->otyp].oc_name_known)
      if (obj->oclass == GEM_CLASS)
         ...
         if (objects[obj->otyp].oc_material == GLASS)
            {
            /* real gem's cost (worthless gems come after jade but
             * before luckstone) */
            tmp = (long) objects[
...
register boolean dummy;
   ...
   bp->bquan = obj->quan;
   if (dummy)
      {                             /* a dummy object must be inserted
                                 * into  */
      bp->useup = 1;              /* the billobjs chain here.  crucial
                                 * for */
      obj->nobj = billobjs;     /* eating floorfood in shop.  see
                                 * eat.c  */
      billobjs = obj;
      }
   else
...
speak:
   if (shkp->mcanmove && !shkp->msleep && !silent)
      ...
      {
      if (ltmp)
         pline ("The list price of %s is %ld zorkmid%s%s.",
                the (xname (obj)), ltmp, plur (ltmp),
                (obj->quan > 1L) ? " each" : "");
      else
...
void
      sellobj_state (deliberate)/* called from dodrop(do.c) and
                                 * doddrop() */
register boolean deliberate;
   {
   /* If we're deliberately dropping something, there's no automatic
    * response to the shopkeeper's "want to sell" query; however, if
    * we accidentally drop anything, the shk will buy it/them without
    * asking. This retains the old pre-query risk that slippery
    * fingers while in shops entailed:  you drop it, you've lost it. */
   ...
   if (shkp->msleep || !shkp->mcanmove)
      ...
      if (!shkp->mcanmove)
         ...
         if (ANGRY (shkp) && !rn2 (4))
            ...
            pline ("%s is indisposed.", Monnam (shkp));
         }
      else if (!rn2 (3))
         {
   ...
   if (!shkp->mgold)
      ...
      if (!sell_response)
         ...
         Sprintf (qbuf,
                  ...
                  xname (obj));
         qlen = strlen (qbuf);
         /* Will the prompt fit on the topline? (or would "--more--"
          * force line wrap anyway?)  If so, combine the message and
          * prompt; otherwise, flush message and prompt separately. */
...
register boolean croaked;
   ...
   register struct damage *tmp_dam,
        *tmp2_dam;
   register boolean did_repair = FALSE,
         saw_door = FALSE;
   register boolean saw_floor = FALSE,
         stop_picking = FALSE;
   uchar saw_walls = 0;
   ...
   while (tmp_dam)
      {
      register xchar x = tmp_dam->place.x,
            y = tmp_dam->place.y;
      char  shops[5];
      uchar disposition;
...
#define vert(i)  ((i/3)-1)
   ...
   if (IS_DOOR (tmp_dam->typ))
      ...
      if (IS_WALL (tmp_dam->typ) && cansee (x, y))
         {
         /* Player sees actual repair process, so they KNOW it's a
          * wall */
         levl[x][y].seen = 1;
         newsym (x, y);
         }
   ...
   if (!fall)
      ...
      if (distu (shkp->mx, shkp->my) > 2)
         ...
         if (distu (shkp->mx, shkp->my) > 2)
            ...
            pline ("%s leaps, and grabs your backpack!",
                   shkname (shkp));
         }
      else
         pline ("%s grabs your backpack!", shkname (shkp));
...
#ifdef KOPS
STATIC_OVL void
      makekops (mm)              /* returns the number of (all types
                                 * of) Kops  made */
coord *mm;
   {
...
const char *dmgstr;
   ...
   if (uinshp)
      ...
      {
      /* Make shkp show up at the door.  Effect:  If there is a
       * monster in the doorway, have the hero hear the shopkeeper
       * yell a bit, pause, then have the shopkeeper appear at the
       * door, having yanked the hapless critter out of the way. */
      if (MON_AT (x, y))
         {
         if (flags.soundok)
            {
...
register struct obj *otmp;
   ...
   if (otmp->oclass != SPBOOK_CLASS && otmp->spe <= 0)
      return;
...
register xchar x,
   ...
   /* Actually, the shk should be made to block _any_ door, including
    * a door the player digs, if the shk is within a 'jumping'
    * distance. */
