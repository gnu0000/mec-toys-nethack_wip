...
boolean talk;
   ...
   if (!xtime && old)
      if (talk)
         You ("feel much less nauseous now.");
...
long  xtime;                    /* nonzero if this is an attempt to
                                 * turn on hallucination */
boolean talk;
long  mask;                       /* nonzero if resistance status
                                 * should change by mask */
   {
   boolean changed = 0;
...
register struct obj *otmp;
   ...
   switch (otmp->otyp)
      ...
      case POT_HALLUCINATION:
         if (Hallucination || HHalluc_resistance)
            nothing++;
      ...
      "This tastes like 10%% real fruit juice all-natural beverage.":
         ...
         if (!otmp->cursed)
            {
            /* Tell them they can see again immediately, which will
             * help them identify the potion... */
      ...
      case POT_GAIN_ABILITY:
         if (otmp->cursed)
            ...
            {                       /* If blessed, increase all; if not,
                                 * try up to */
            int   itmp;           /* 6 times to find one which can be
                                 * increased. */
      ...
      switch (obj->otyp)
         ...
         case POT_PARALYSIS:
            if (mon->mcanmove)
               {
               mon->mcanmove = 0;
               /* really should be rnd(5) for consistency with
                * players breathing potions, but... */
               mon->mfrozen = rnd (25);
               }
            break;
         case POT_SPEED:
