   ...
   if (ep && ep->engr_txt[0])
      ...
      switch (ep->engr_type)
         ...
         case MARK:
            ...
            break;
         case BLOOD:
            /* "It's a message!  Scrawled in blood!" "What's it say?"
...
int
      doengrave ()
   {
   boolean dengr = FALSE;        /* TRUE if we wipe out the current
                                 * engraving */
   boolean doblind = FALSE;     /* TRUE if engraving blinds the
                                 * player */
   boolean doknown = FALSE;     /* TRUE if we identify the stylus */
   boolean eow = FALSE;           /* TRUE if we are overwriting oep */
   boolean jello = FALSE;        /* TRUE if we are engraving in slime */
   boolean ptext = TRUE;        /* TRUE if we must prompt for engrave
                                 * text */
   boolean teleengr = FALSE;     /* TRUE if we move the old engraving */
   boolean zapwand = FALSE;     /* TRUE if we remove a wand charge */
   xchar type = DUST;           /* Type of engraving made */
   char  buf[BUFSZ];              /* Buffer for final/poly engraving
                                 * text */
   char  ebuf[BUFSZ];           /* Buffer for initial engraving text */
   char  qbuf[QBUFSZ];           /* Buffer for query text */
   char  post_engr_text[BUFSZ];  /* Text displayed after engraving
                                 * prompt */
   const char *everb;           /* Present tense of engraving type */
   const char *eloc;              /* Where the engraving is (ie
                                 * dust/floor/...) */
   register char *sp;           /* Place holder for space count of
                                 * engr text */
   register int len;              /* # of nonspace chars of new
                                 * engraving text */
   register int maxelen;        /* Max allowable length of new
                                 * engraving text */
   register int spct;           /* # of spaces in new engraving text */
   register struct engr *oep = engr_at (u.ux, u.uy);

   /* The current engraving */
   register struct obj *otmp;     /* Object selected with which to
                                 * engrave */
   ...
   /* There's no reason you should be able to write with a wand while
    * both your hands are tied up. */
   ...
   switch (otmp->oclass)
      ...
      case RANDOM_CLASS:        /* This should mean fingers */
          ...
          * and pre-engraving messages are based upon knowing what
          * type of engraving the wand is going to do.  Also, the
          * player will have potentially seen "You wrest .." message,
          * and therefore will know they are using a charge. */
      case WAND_CLASS:
         if (zappable (otmp))
            {
            check_unpaid (otmp);
   ...
   if (oep)
      ...
      if (c == 'n' || Blind)
         ...
         else if ((type == DUST) || (type == MARK) || (type == BLOOD))
            {
            You (
   ...
   switch (type)
      ...
      case ENGRAVE:
         ...
         if ((otmp->oclass == WEAPON_CLASS) &&
            ...
            /* -2 = 3, -1 = 5, 0 = 7, +1 = 9, +2 = 11 Note: this does
             * not allow a +0 anything (except an athame) to engrave
             * "Elbereth" all at once. However, you could now engrave
...
int   fd;
   ...
   while (1)
      ...
      /* mark as finished for bones levels -- no problem for normal
       * levels as the player must have finished engraving to be able
       * to move again */
      ep->engr_time = moves;
      }
   }
