...
uchar oc_syms[MAXOCLASSES] = DUMMY;   /* the current object  display
                                     * symbols */
uchar showsyms[MAXPCHARS] = DUMMY;   /* the current feature display
                                     * symbols */
uchar monsyms[MAXMCLASSES] = DUMMY;   /* the current monster display
                                     * symbols */
...
#ifdef PC9801
void  NDECL ((*ibmgraphics_mode_callback)) = 0;   /* set in
                                                 * tty_start_screen() */
...
static uchar ibm_graphics[MAXPCHARS] =    {
   ...
   0xfe,                          /* S_vodoor:   meta-~, small centered
                                 * square */
   0xfe,                          /* S_hodoor:   meta-~, small centered
                                 * square */
   g_FILLER (S_vcdoor),
   g_FILLER (S_hcdoor),
   0xfa,                          /* S_room:   meta-z, centered dot */
   0xb0,                          /* S_corr:   meta-0, light shading */
   0xb1,                          /* S_litcorr:   meta-1, medium
                                 * shading */
   ...
   g_FILLER (S_throne),
   g_FILLER (S_sink),
   0xf4,                          /* S_fountain:  meta-t, integral top
                                 * half */
...
#ifdef TERMLIB
void  NDECL ((*decgraphics_mode_callback)) = 0;   /* set in
                                                 * tty_start_screen() */
...
static uchar dec_graphics[MAXPCHARS] =    {
   ...
   g_FILLER (S_dnladder),
   g_FILLER (S_trap),
   g_FILLER (S_web),              /* 0xbd, /* \E)3: meta-=, int'l
                                 * currency */
   g_FILLER (S_altar),           /* 0xc3, /* \E)3: meta-C, dagger */
   g_FILLER (S_throne),
   g_FILLER (S_sink),
   g_FILLER (S_fountain),        /* 0xdb, /* \E)3: meta-[, integral
                                 * top half */
   ...
   g_FILLER (S_ss4),
   g_FILLER (S_sw_tl),
   0xef,                          /* S_sw_tc:   meta-o, high horizontal
                                 * line */
   g_FILLER (S_sw_tr),
   0xf8,                          /* S_sw_ml:   meta-x, vertical rule */
   0xf8,                          /* S_sw_mr:   meta-x, vertical rule */
   g_FILLER (S_sw_bl),
   0xf3,                          /* S_sw_bc:   meta-s, low horizontal
                                 * line */
   g_FILLER (S_sw_br),
    /*60*/ g_FILLER (S_explode1),
   0xef,                          /* S_explode2:   meta-o, high
                                 * horizontal line */
   g_FILLER (S_explode3),
...
#ifdef PC9801
void  NDECL ((*ascgraphics_mode_callback)) = 0;   /* set in
                                                 * tty_start_screen() */
