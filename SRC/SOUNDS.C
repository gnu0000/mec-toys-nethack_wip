...
void
   ...
   if (level.flags.has_court && !rn2 (200))
      ...
      if (which != 2)
         You (throne_msg[which]);
      else
         pline (throne_msg[2], his[flags.female]);
      return;
      }
...
register struct monst *mtmp;
   ...
   if (mtmp->msleep || !mtmp->mcanmove || !mtmp->data->msound)
      return;
   ...
   if (growl_verb)
      pline ("%s %ss!", Monnam (mtmp), growl_verb);
   }
...
register struct monst *mtmp;
   {
#ifdef SOUNDS
   register const char *pline_msg = 0;   /* Monnam(mtmp) will be
                                        * prepended */
   ...
   switch (mtmp->data->msound)
      ...
      case MS_DJINNI:
         if (mtmp->mtame)
            ...
            verbalize ("This will teach you not to disturb me!");
         break;
      case MS_HUMANOID:
         if (!mtmp->mpeaceful)
            {
...
# endif
                  ...
                  verbalize ("Relax, this won't hurt a bit.");
               break;
            case MS_GUARD:
               if (u.ugold)
   ...
   static int
      ...
      if (!Blind && (otmp = shop_object (u.ux, u.uy)) != (struct obj *) 0)
         {
         /* standing on something in a shop and chatting causes the
          * shopkeeper to describe the price(s).  This can inhibit
          * other chatting inside a shop, but that shouldn't matter
          * much.  shop_object() returns an object iff inside a shop
          * and the shopkeeper is present and willing (not angry) and
          * able (not asleep) to speak and the position contains any
          * objects other than just gold. */
         price_quote (otmp);
         return (1);
         }
