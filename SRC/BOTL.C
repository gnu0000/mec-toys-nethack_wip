...
#else                             /* OVLB */
STATIC_OVL NEARDATA int mrank_sz = 0;   /* loaded by max_rank_sz (from
                                        * u_init) */
...
struct class_ranks all_classes[] =    {
      ...
      {'E', 0, PM_ELF,    {
            ...
            {"Ohtar", "Ohtie"   },     /* warrior */
            {"Kano",                 /* commander (Q.) ['a] */
         "Kanie"   },              /* educated guess, until further
                                 * research- SAC */
            {"Arandur",              /* king's servant, minister (Q.) -
                                 * guess */
...
char  pc;
   ...
   for (i = 0; i < SIZE (all_classes); i++)
      if (all_classes[i].plclass == pc)
         return all_classes[i].titles;
   return 0;
   }
...
void
   ...
   if (ranks)
      ...
      for (i = 0; i < 9; i++)
         {
         if ((r = strlen (ranks[i].m)) > maxr)
            maxr = r;
         if (ranks[i].f)
            if ((r = strlen (ranks[i].f)) > maxr)
               maxr = r;
         }
      mrank_sz = maxr;
      }
...
static void
   ...
   Sprintf (nb = eos (nb), "  ");
   i = mrank_sz + 15;
   j = (nb + 2) - newbot1;        /* aka strlen(newbot1) but less
                                 * computation */
   if ((i - j) > 0)
