...
#define NEED_VARARGS              /* comment line for pre-compiled
                                 * headers */
...
# ifdef SYSV
   (void)
# endif
      abort ();                 /* avoid loops - this should never
                                 * happen */
#endif
...
boolean taken;
   ...
   if (invent && !done_stopprint &&
      ...
      if (taken)
         {
         /* paybill has already given the inventory locations in the
          * shop and put it on the main object list */
         struct obj *obj;
...
int   how;
   ...
   /* kilbuf: used to copy killer in case it comes from something
    * like xname(), which would otherwise get overwritten when we
    * call xname() when listing possessions pbuf: holds Sprintf'd
    * output for raw_print and putstr */
   if (how == ASCENDED)
   ...
   /* Sometimes you die on the first move.  Life's not fair. On those
    * rare occasions you get hosed immediately, go out smiling... :-)
    * -3. */
...
/* note that killer is pointing at kilbuf */
   ...
   /* paybill() must be called unconditionally, or strange things
    * will happen to bones levels */
   ...
   if (have_windows)
      ...
      if (how < GENOCIDED && flags.tombstone)
         outrip (endwin, how);
      }
   else
...
*/
   ...
   if (how == ESCAPED || how == ASCENDED)
      ...
      /* Put items that count into the jewels chain.  Rewriting the
       * invent chain and all the container chains (within invent)
       * here is safe.  They will never be used again. */
      ...
      if (how != ASCENDED)
         ...
         if (Is_astralevel (&u.uz))
            ...
            Strcat (pbuf, dungeons[u.uz.dnum].dname);
         Strcat (pbuf, " ");
         if (!In_endgame (&u.uz)
#ifdef MULDGN
             && !Is_knox (&u.uz)
   ...
   if (have_windows)
      exit_nhwindows (NULL);
   /* "So when I die, the first thing I will see in Heaven is a score
    * list?" */
   topten (how);
   if (done_stopprint)
      {
...
static void
   ...
   /* vanquished foes list; includes all dead monsters, not just
    * those killed by the player */
   if (ntypes != 0)
      {
      ...
      if (c == 'y')
         ...
         /* if (Hallucination) putstr(klwin, 0, "and a partridge in a
          * pear tree"); */
         if (ntypes > 1)
            {
            putstr (klwin, 0, "");
