...
static char circle_data[] =    {
    ...
    /*135*/ 16                    /* should be MAX_RADIUS+1; used to
                                 * terminate range loops -dlc */
   };
...
static char circle_start[] =    {
    /*  */ 0,                    /* circles of radius zero are not
                                 * used */
    /* 1*/ 0,
    /* 2*/ 2,
    /* 3*/ 5,
...
#endif
char *viz_rmin,
     *viz_rmax;                 /* current vision cs bounds */
...
#ifdef VISION_TABLES
   /* Note:  this initializer doesn't do anything except guarantee
    * that we're linked properly. */
   vis_tab_init ();
#endif
   }
...
void
   ...
   for (y = 0; y < ROWNO; y++)
      {
      dig_left = 0;
      block = TRUE;              /* location (0,y) is always stone;
                                 * it's !isok() */
      lev = &levl[1][y];
      ...
      /* handle right boundary; almost identical for
       * blocked/unblocked */
      i = dig_left;
      if (!block && dig_left)
         dig_left--;              /* point at first blocked point */
...
char *rmin,
   ...
   for (zy = ylo; zy <= yhi; zy++)
      ...
      for (zx = xlo; zx <= xhi; zx++)
         {
         next[zy][zx] = COULD_SEE | IN_SIGHT;
         /* Yuck, update adjacent non-diagonal positions when in a
          * doorway. We need to do this to catch the case when we
          * first step into a room.  The room's walls were not seen
          * from the outside, but now are seen (the seen bit is set
          * just above).  However, the positions are not updated
          * because they were already in sight. So, we have to do it
          * here. */
...
int   control;
   ...
   char *ranges;                 /* circle ranges -- used for xray &
                                 * night vision */
   int   row;                    /* row counter (outer loop)  */
   int   start,
         stop;                    /* inner loop starting/stopping index */
   int   dx,
         dy;                    /* one step from a lit door or lit
                                 * wall (see below) */
   register int col;              /* inner loop counter */
   register struct rm *lev;     /* pointer to current pos */
   struct rm *flev;              /* pointer to position in "front" of
                                 * current pos */
   ...
   /* You see nothing, nothing can see you --- if swallowed or
    * refreshing. */
   if (u.uswallow || control == 2)
      ...
      {
      /* Calculate the could_see array even when blind so that
       * monsters can see you, even if you can't see them.  Note that
      ...
      /* Our own version of the update loop below.  We know we can't
       * see anything, so we only need update positions we used to be
       * able to see. */
      temp_array = viz_array;     /* set viz_array so newsym() will
                                 * work */
      ...
      if (Underwater && !Is_waterlevel (&u.uz))
         {
         /* The hero is under water.  Only see surrounding locations
          * if they are also underwater.  This overrides night vision
          * but does not override x-ray vision. */
         has_night_vision = 0;
   ...
   /* Make the viz_array the new array so that cansee() will work
    * correctly. */
    ...
    * difficult:
    * 
    * 1.  Walls.  Walls are only seen as walls from the inside of a
    * room. On the outside they look like stone.  The "seen" bit in
    * the rm structure is used in the display system to decide what
    * to display, but it is here where we decide to set the seen bit.
    ...
    * The worst offenders are doors.  Suppose a door to a lit room is
    * closed.  It is lit on one side, but not on the other.  How do
    * you know?  You have to check the closest adjacent position.
      ...
      for (col = start; col <= stop; col++, lev += ROWNO)
         ...
         if (next_row[col] & IN_SIGHT)
            ...
            if ((!lev->seen || (lev->diggable & W_REPAIRED)) &&
               ...
               /* If it was a non-corridor "open" area, we see the
                * wall */
             ...
             * 
             * It is assumed here that lit walls are lit from the inside
             * of the room,  Hence, walls are not "seen" unless we
             * can see them from across a lit room square. */
            if (IS_WALL (lev->typ) || lev->typ == SDOOR)
                ...
                * at the end of dark hallways.  We do this by
                * checking the adjacent position.  If it is lit, then
                * we can see the door, otherwise we can't. */
               dx = u.ux - col;
               dx = sign (dx);
               flev = &(levl[col + dx][row + dy]);
               if (flev->lit)
             ...
             * (next_row[col] is true), but doesn't see it (lit is
             * false). However, the hero _remembers_ it as lit
             * (waslit is true). The hero can now see that it is not
             * lit, so change waslit and update the location. */
...
int   x,
    ...
    * hero could see a lit room.  However, the position of the
    * opening was out of night-vision range of the hero.  Suddenly
    * the hero should see the lit room. */
   if (viz_array[y][x])
...
int   side,
    ...
    * 
    * If the block is beyond the border, then the row is negative.
    * Return the block's column number (should be 0 or COLNO-1).
    * 
    * Could easily have the column be -1, but then wouldn't know if it
    * was the left or right border. */
   if (block_row < 0)
      return block_col;
...
int   row;                       /* current row */
int   cb_row,
      cb_col;                    /* close block row and col */
int   fb_row,
      fb_col;                    /* far block row and col */
int   left;                       /* left mark of the previous row */
int   right_mark;                 /* right mark of previous row */
char *limits;                    /* points at range limit for current
                                 * row, or NULL */
   {
   register int i;
   register char *rowp;
   ...
   /* Mark all stone walls as seen before the left shadow.  All this
    * work for a special case.
    * 
    ...
    * change the above assignment so that left and not left_shadow is
    * the variable that gets the shadow. */
   while (left <= right_mark)
      ...
      if (loc_right > right_mark)   /* can't see stone beyond the
                                     * mark */
   ...
   /* At this point we are at the first visible clear spot on or
    * beyond the left shadow, unless the left shadow is an illegal
    * value.  If we have "hit stone" then we have a stone wall just
    * to our left. */
   ...
   /* Make vertical walls work the way we want them.  In this case,
    * we note when the close block blocks the column just
    * above/beneath it (right_shadow < fb_col [actually right_shadow
    * == fb_col-1]).  If the location is filled, then we want to see
    * it, so we put the right shadow back (same as fb_col). */
   if (right_shadow < fb_col && !viz_clear_rows[row][fb_col])
      right_shadow = fb_col;
   if (right_shadow > lim_max)
      right_shadow = lim_max;

   /* Main loop.  Within the range of sight of the previous row, mark
    * all stone walls as seen.  Follow open areas recursively. */
   while (left <= right_mark)
      {
      ...
      if (!viz_clear_rows[row][left])
         {
         hit_stone = 1;           /* use stone on this row as close
                                 * block */
         /* We can see all of the wall until the next open spot or
          * the start of the shadow caused by the far block (right).
          * 
       ...
       * 
       * If this is the first open spot since the could see area  (this
       * is true if we have hit stone), get the shadow generated by
       * the wall just to our left. */
      if (hit_stone)
         {
       ...
       * wall on this row does *not* cover the wall on the right of
       * the opening. */
      if (left >= loc_right)
         {
         if (loc_right == lim_max)
      ...
      /* If the far wall of the opening (loc_right) is closer than
       * the shadow limit imposed by the far block (right) then use
       * the far wall as our new far block when we recurse.
       * 
       * If the limits are the the same, and the far block really exists
       * (fb_row >= 0) then do the same as above.
       * 
       * Normally, the check would be for the far wall being closer OR
       * EQUAL to the shadow limit.  However, there is a bug that
       * arises from the fact that the clear area pointers end in an
       * open space (if it exists) on a boundary.  This then makes a
       * far block exist where it shouldn't --- on a boundary.  To
       * get around that, I had to introduce the concept of a
       * non-existent far block (when the row < 0).  Next I have to
       * check for it.  Here is where that check exists. */
      if ((loc_right < right_shadow) ||
         ...
         /* The following line, setting hit_stone, is needed for
          * those walls that are only 1 wide.  If hit stone is *not*
          * set and the stone is only one wide, then the close block
          * is the old one instead one on the current row.  A way
          * around having to set it here is to make left = loc_right
          * (not loc_right+1) and let the outer loop take care of it.
          * However, if we do that then we then have to check for
          * boundary conditions here as well. */
         hit_stone = 1;
...
char *limits;
   ...
   while (right >= left_mark)
      ...
      /* If the far wall of the opening is closer than the shadow
       * limit. */
...
int   srow,
      scol;                       /* source row and column */
char **loc_cs_rows;              /* could_see array (row pointers) */
char *left_most,
     *right_most;                 /* limits of what could be seen */
...
int   row;                       /* current row */
int   left;                       /* first (left side) visible spot on
                                 * prev row */
int   right_mark;                 /* last (right side) visible spot on
                                 * prev row */
char *limits;                    /* points at range limit for current
                                 * row, or NULL */
   {
   ...
   char *row_min;                 /* left most  [used by macro
                                 * set_min()] */
   char *row_max;                 /* right most [used by macro
                                 * set_max()] */
   int   lim_max;                 /* right most limit of circle */
...
#endif
   nrow = row + step;
   /* Can go deeper if the row is in bounds and the next row is
    * within the circle's limit.  We tell the latter by checking to
    * see if the next limit value is the start of a new circle radius
    * (meaning we depend on the structure of circle_data[]). */
   deeper = good_row (nrow) && (!limits || (*limits >= *(limits + 1)));
   if (!vis_func)
      {
   ...
   while (left <= right_mark)
      ...
      if (!is_clear (row, left))
         {
         /* Jump to the far side of a stone wall.  We can set all the
          * points in between as seen.
          * 
         ...
         if (right_edge > right_mark)
             ...
             * beyond the mark on this row.  If so, then see one
             * beyond the mark.  Otherwise don't.  This is a kludge
             * so corners with an adjacent doorway show up in
             * nethack. */
      ...
      if (left != start_col)
         {
         /* Find the left side.  Move right until we can see it or we
          * run into a wall. */
      ...
      rside1:                    /* used if q?_path() is a macro */
         ...
         /* Check for boundary conditions.  We *need* check (2) to
          * break an infinite loop where:
          * 
          ...
          * (left == right_edge) or might not (left == right_edge+1)
          * have been able to see the far wall.  Make sure we *can*
          * see the wall (remember, we can see the spot above/below
          * this one) by backing up. */
         if (left >= right_edge)
            {
       ...
       * closer than the edge on this row, then we have to check how
       * far we can see around the corner (under the overhang).  Stop
       * at the first non-visible spot or we actually hit the far
       * wall.
       * 
       * Otherwise, we know we can see the right edge of the current
       * row.
       * 
      ...
      if (left <= right)
         {
         /* An ugly special case.  If you are adjacent to a vertical
          * wall and it has a break in it, then the right mark is set
          * to be start_col.  We *want* to be able to see adjacent
          * vertical walls, so we have to set it back. */
...
char **loc_cs_rows;              /* pointers to the rows of the
                                 * could_see array */
...
genericptr_t arg;
   ...
   /* Set globals for q?_path(), left_side(), and right_side() to
    * use. */
   start_col = scol;
   start_row = srow;
   ...
   if (is_clear (srow, scol))
       ...
       * unless you are on an array boundary or a stone/clear
       * boundary. */
      left = (!scol) ? 0 :
   ...
   /* Check what could be seen in quadrants.  We need to check for
    * valid rows here, since we don't do it in the routines
    * right_side() and left_side() [ugliness to remove extra routine
    * calls]. */
   if ((nrow = srow + 1) < ROWNO)
      {                             /* move down */
      step = 1;
