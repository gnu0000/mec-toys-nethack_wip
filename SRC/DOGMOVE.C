...
STATIC_VAR xchar gtyp,
      gx,
      gy;                       /* type and position of dog's current
                                 * goal */
...
int   x,
   ...
   /* It is arbitrary that the pet takes the same length of time to
    * eat as a human, but gets more nutritional value. */
   if (obj->oclass == FOOD_CLASS)
       ...
       * meating made consistent with wild monsters in mon.c. nutrit
       * made consistent with polymorphed player nutrit in eat.c.
       * (This also applies to pets eating gold.) */
...
int
      dog_move (mtmp, after)
register struct monst *mtmp;
register int after;              /* this is extra fast monster
                                 * movement */
   {
   int   omx,
         omy;                    /* original mtmp position */
   int   appr,
         whappr,
         udist;
   int   i,
   ...
   boolean has_edog,
         ...
         do_eat = FALSE;
   xchar nix,
         niy;                    /* position mtmp is (considering)
                                 * moving to */
   register int nx,
         ny;                    /* temporary coordinates */
   xchar cnt,
         uncursedcnt,
         chcnt;
   int   chi = -1,
...
#define GDIST(x,y) (dist2(x,y,gx,gy))
   ...
   /* Tame Angels have isminion set and an ispriest structure instead
    * of an edog structure.  Fortunately, guardian Angels need not
    * worry about mundane things like eating and fetching objects,
    * and can spend all their energy defending the player.  (They are
    * the only monsters with other structures that can be tame.) */
   ...
   if (Conflict && !resist (mtmp, RING_CLASS, 0, 0))
      ...
      if (!has_edog)
         ...
         /* Guardian angel refuses to be conflicted; rather, it
          * disappears, angrily, and sends in some nasties */
...
skipu:;
   ...
   for (i = 0; i < cnt; i++)
      ...
      if ((info[i] & ALLOW_M) && MON_AT (nx, ny))
         ...
         {                          /* dog avoids traps, but perhaps it
                                 * has to pass a trap in order to
                                 * follow player */
         struct trap *trap;
...
#ifdef WALKIES
   /* an incredible kludge, but the only way to keep pooch near after
    * it spends time eating or in a trap, etc. */
      ...
      cc.x = u.ux + nx;
      cc.y = u.uy + ny;
      if (goodpos (cc.x, cc.y, mtmp, mtmp->data))
         goto dognext;
